...
object DeploymentResolver {
  def resolve(config: RiffRaffDeployConfig)
    ...
    config.deployments
      .traverseU[Validated[ConfigErrors, PartiallyResolvedDeployment]] {
        case (label, rawDeployment) =>
          for {
            templated <- applyTemplates(label, rawDeployment, config.templates)
            deployment <- resolveDeployment(label,
                                            templated,
                                            config.stacks,
                                            config.regions)
            validatedDeployment <- validateDependencies(label,
                                                        deployment,
                                                        config.deployments)
  ...
  /**
    ...
    */
  private[input] def resolveDeployment(label: String,
    ...
    (Validated.fromOption(templated.`type`,
      ...
      Validated.fromOption(
        ...
        ConfigErrors(label, "No regions provided"))) map {
      (deploymentType, stacks, regions) =>
    ...
    template.template match {
      ...
      case Some(parentTemplateName) =>
        for {
          ...
          resolvedParent <- applyTemplates(parentTemplateName,
                                           parentTemplate,
                                           templates)
        } yield {
          DeploymentOrTemplate(
            ...
            contentDirectory = template.contentDirectory.orElse(
              resolvedParent.contentDirectory),
            dependencies =
              template.dependencies.orElse(resolvedParent.dependencies),
            parameters = Some(
              resolvedParent.parameters
                .getOrElse(Map.empty) ++ template.parameters.getOrElse(
                Map.empty))
          )
        }
    }
      ...
      case missingDependencies =>
        Invalid(
          ConfigErrors(
            label,
            missingDependencies
              .mkString(s"Missing deployment dependencies ", ", ", "")))
    }
  }
}
