    ...
    val edges = for {
      ...
      edge <- {
        val dependents = dependency match {
          case StartNode =>
            deploymentNodes.filterNot(
              _.value.dependencies.exists(deployments.map(_.name).contains))
          case ValueNode(deployment) =>
            deploymentNodes.filter(
              _.value.dependencies.contains(deployment.name))
          case EndNode =>
            throw new IllegalStateException(
              "EndNode is not valid as the source of an edge")
        }
