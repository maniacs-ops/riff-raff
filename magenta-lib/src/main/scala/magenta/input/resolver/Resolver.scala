...
object Resolver {
  def resolve(yamlConfig: String,
    ...
    for {
      deploymentGraph <- resolveDeploymentGraph(yamlConfig,
                                                deploymentTypes,
                                                parameters.selector)
      taskGraph <- buildTaskGraph(deploymentResources,
                                  parameters,
                                  deploymentTypes,
                                  artifact,
                                  deploymentGraph)
  ...
  def resolveDeploymentGraph(yamlConfig: String,
    ...
    for {
      ...
      validatedDeployments <- deployments
        .traverseU[Validated[ConfigErrors, Deployment]] { deployment =>
          DeploymentTypeResolver.validateDeploymentType(deployment,
                                                        deploymentTypes)
        }
  ...
  private[resolver] def buildTaskGraph(
    ...
    val deploymentTaskGraph = flattenedGraph.map { deployment =>
      TaskResolver.resolve(deployment,
                           deploymentResources,
                           parameters,
                           deploymentTypes,
                           artifact)
