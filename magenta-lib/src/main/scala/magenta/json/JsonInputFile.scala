...
object JsonInputFile {
  ...
  def parse(
      s: String): Either[Seq[(JsPath, Seq[ValidationError])], JsonInputFile] =
...
object JsonReader {
  ...
  def defaultRecipes(packages: Map[String, DeploymentPackage]) =
    Map(
      "default" -> JsonRecipe(
        actions = Some(packages.values.map(_.name + ".deploy").toList)))

  def buildProject(input: JsonInputFile,
                   artifact: S3JsonArtifact,
                   deploymentTypes: Seq[DeploymentType]): Project = {
    val packages = input.packages map {
      case (name, pkg) =>
        name -> parsePackage(name, pkg, artifact, deploymentTypes)
    }
    ...
    Project(packages,
            recipes,
            input.defaultStacks.getOrElse(Nil).map(NamedStack(_)))
  }

  private def parseRecipe(
      name: String,
      jsonRecipe: JsonRecipe,
      availablePackages: Map[String, DeploymentPackage]) = {
      ...
      actionString.split("\\.") match {
        case Array(pkgName, actionName) =>
          val pkg = availablePackages
            .get(pkgName)
            .getOrElse(sys.error(
              s"Package '$pkgName' does not exist; cannot resolve action '$actionString'"))
          pkg.mkDeploymentStep(actionName)

        case _ =>
          sys.error(
            s"Badly formed action name: '$actionString' - should be in <packageName>.<actionName> format")
      }
    }
