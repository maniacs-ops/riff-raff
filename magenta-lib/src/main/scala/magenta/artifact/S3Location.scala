...
trait S3Location {
  ...
  def fetchContentAsString()(implicit client: AmazonS3) =
    S3Location.fetchContentAsString(this)
  def listAll()(implicit client: AmazonS3) = S3Location.listObjects(this)
}

object S3Location extends Loggable {
  def listAll(bucket: String)(implicit s3Client: AmazonS3): Seq[S3Object] =
    listObjects(bucket, None)

  def listObjects(location: S3Location)(
      implicit s3Client: AmazonS3): Seq[S3Object] =
  ...
  private def listObjects(bucket: String, prefix: Option[String])(
      implicit s3Client: AmazonS3): Seq[S3Object] = {
    ...
    @tailrec
    def pageListings(
        acc: Seq[ListObjectsV2Result],
        previousListing: ListObjectsV2Result): Seq[ListObjectsV2Result] = {
  ...
  def fetchContentAsString(location: S3Location)(
      implicit client: AmazonS3): Either[S3Error, String] = {
    import cats.syntax.either._
    Either
      .catchNonFatal(client.getObjectAsString(location.bucket, location.key))
      .leftMap {
        case e: AmazonS3Exception if e.getStatusCode == 404 =>
          EmptyS3Location(location)
...
object S3JsonArtifact extends Loggable {
  ...
  def fetchInputFile(artifact: S3JsonArtifact)(implicit client: AmazonS3,
                                               reporter: DeployReporter)
    : Either[ArtifactResolutionError, JsonInputFile] = {

    import cats.syntax.either._
    val possibleJson =
      (artifact.deployObject.fetchContentAsString()(client)).orElse {
  ...
  def convertFromZipBundle(artifact: S3JsonArtifact)(
      implicit client: AmazonS3,
      reporter: DeployReporter): Unit = {
    reporter.warning(
    ...
    S3ZipArtifact.withDownload(artifact) { dir =>
      ...
      reporter.info(
        s"Uploading contents of artifact (${filesToUpload.size} files) to S3")
      filesToUpload.foreach {
        case (file, key) =>
          val metadata = new ObjectMetadata()
          metadata.setContentLength(file.length)
          val req = new PutObjectRequest(artifact.bucket, key, file)
  ...
  private def resolveFiles(file: File, key: String): Seq[(File, String)] = {
    if (!file.isDirectory) Seq((file, key))
      ...
      file.listFiles.toSeq
        .flatMap(f => resolveFiles(f, subDirectoryPrefix(key, f)))
        .distinct
  }
}
...
case class JsonArtifactError(underlying: Seq[(JsPath, Seq[ValidationError])])
    extends ArtifactResolutionError
