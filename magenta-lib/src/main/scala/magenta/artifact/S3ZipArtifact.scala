...
object S3ZipArtifact {
  def download(artifact: S3JsonArtifact)(implicit client: AmazonS3,
                                         reporter: DeployReporter): File = {
    val dir = Path.createTempDirectory(prefix = "riffraff-", suffix = "")
    download(artifact, dir)(client, reporter)
    dir
  }

  def download(artifact: S3JsonArtifact, dir: File)(implicit client: AmazonS3,
    ...
    val artifactPath =
      Path.createTempFile(prefix = "riffraff-artifact-", suffix = ".zip")
    try {
      val blob = Resource.fromInputStream(
        client.getObject(path.bucket, path.key).getObjectContent)
      blob.copyDataTo(artifactPath)

      CommandLine(
        "unzip" :: "-q" :: "-d" :: dir.getAbsolutePath :: artifactPath.getAbsolutePath :: Nil)
        .run(reporter)
  ...
  def withDownload[T](artifact: S3JsonArtifact)(
      block: File => T)(client: AmazonS3, reporter: DeployReporter): T = {
