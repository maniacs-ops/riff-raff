...
import com.amazonaws.services.elasticloadbalancing.{
  AmazonElasticLoadBalancing => ClassicELB,
  AmazonElasticLoadBalancingClientBuilder => ClassicELBBuilder
}
import com.amazonaws.services.elasticloadbalancingv2.{
  AmazonElasticLoadBalancing => ApplicationELB,
  AmazonElasticLoadBalancingClientBuilder => ApplicationELBBuilder
}
...
object S3 extends AWS {
  def makeS3client(
      keyRing: KeyRing,
      region: Region,
      config: ClientConfiguration = clientConfiguration): AmazonS3 =
  ...
  /**
    ...
    * @return
    */
  def accountSpecificBucket(prefix: String,
                            s3Client: AmazonS3,
                            stsClient: AWSSecurityTokenService,
                            region: Region,
                            reporter: DeployReporter,
                            deleteAfterDays: Option[Int] = None): String = {
    val callerIdentityResponse =
    ...
    if (!s3Client.doesBucketExist(bucketName)) {
      ...
      val createBucketRequest = region.name match {
        case "us-east-1" =>
          new CreateBucketRequest(bucketName) // this needs to be special cased as setting this explicitly blows up
...
object Lambda extends AWS {
  ...
  def lambdaUpdateFunctionCodeRequest(
      functionName: String,
      buffer: ByteBuffer): UpdateFunctionCodeRequest = {
...
object ASG extends AWS {
  ...
  def isStabilized(asg: AutoScalingGroup,
    ...
    def matchCapacityAndState(
        states: List[String],
        desiredState: String,
        checkDescription: Option[String]): Either[String, Unit] = {
      val descriptionWithPrecedingSpace =
        checkDescription.map(d => s" $d").getOrElse("")
      if (states.size != asg.getDesiredCapacity)
        Left(
          s"Number of$descriptionWithPrecedingSpace instances (${states.size}) and ASG desired capacity (${asg.getDesiredCapacity}) don't match")
    ...
    (elbName(asg), elbTargetArn(asg)) match {
      ...
      case (None, Some(arn)) => {
        ...
        matchCapacityAndState(elbHealth,
                              TargetHealthStateEnum.Healthy.toString,
                              Some("Application ELB"))
      }
      case (None, None) => {
        val instanceStates =
          asg.getInstances.asScala.toList.map(_.getLifecycleState)
        matchCapacityAndState(instanceStates,
                              LifecycleState.InService.toString,
                              None)
      }
      case (Some(_), Some(_)) =>
        Left(
  ...
  def elbName(asg: AutoScalingGroup) =
    asg.getLoadBalancerNames.asScala.headOption

  def elbTargetArn(asg: AutoScalingGroup) =
    asg.getTargetGroupARNs.asScala.headOption
  ...
  def suspendAlarmNotifications(name: String, client: AmazonAutoScaling) =
  ...
  def resumeAlarmNotifications(name: String, client: AmazonAutoScaling) =
  ...
  def groupForAppAndStage(pkg: DeploymentPackage,
    ...
    implicit class RichAutoscalingGroup(asg: AutoScalingGroup) {
      def hasTag(key: String, value: String) = asg.getTags.asScala exists {
        tag =>
      ...
      def matchApp(app: App, stack: Stack): Boolean = {
        stack match {
          case UnnamedStack =>
            hasTag("Role", pkg.name) || hasTag("App", pkg.name)
          case NamedStack(stackName) =>
            hasTag("Stack", stackName) && hasTag("App", app.name)
        }
      }
    }

    def listAutoScalingGroups(
        nextToken: Option[String] = None): List[AutoScalingGroup] = {
      ...
      Option(result.getNextToken) match {
        ...
        case token: Some[String] =>
          autoScalingGroups ++ listAutoScalingGroups(token)
      }
    }
    ...
    val appMatch: ASGMatch = appToMatchingGroups match {
      case Seq() =>
        reporter.fail(
          s"No autoscaling group found in ${stage.name} with tags matching package ${pkg.name}")
      case Seq(onlyMatch) => onlyMatch
      case firstMatch +: otherMatches =>
        reporter.info(
          s"More than one app matches an autoscaling group, the first in the list will be used")
        firstMatch
    }
    ...
    appMatch match {
      case ASGMatch(_, List(singleGroup)) =>
        reporter.verbose(
          s"Using group ${singleGroup.getAutoScalingGroupName} (${singleGroup.getAutoScalingGroupARN})")
        singleGroup
      case ASGMatch(app, groupList) =>
        reporter.fail(
          s"More than one autoscaling group match for $app in ${stage.name} (${groupList.map(_.getAutoScalingGroupARN).mkString(", ")}). Failing fast since this may be non-deterministic.")
    }
  }
}
...
object ELB extends AWS {
  ...
  def targetInstancesHealth(targetARN: String,
                            client: ApplicationELB): List[String] =
    client
      .describeTargetHealth(
        new DescribeTargetHealthRequest().withTargetGroupArn(targetARN))
  ...
  def instanceHealth(elbName: String, client: ClassicELB): List[String] =
    client
      .describeInstanceHealth(new DescribeInstanceHealthRequest(elbName))
      .getInstanceStates
      .asScala
      .toList
      .map(_.getState)

  def deregister(elbName: Option[String],
                 elbTargetARN: Option[String],
                 instance: ASGInstance,
                 client: Client) = {
    elbName.foreach(
      name =>
        client.classic.deregisterInstancesFromLoadBalancer(
          new DeregisterInstancesFromLoadBalancerRequest()
            ...
            .withInstances(
              new ELBInstance().withInstanceId(instance.getInstanceId))))
    elbTargetARN.foreach(arn =>
      client.application.deregisterTargets(new DeregisterTargetsRequest()
        .withTargetGroupArn(arn)
...
object EC2 extends AWS {
  ...
  def setTag(instances: List[ASGInstance],
             key: String,
             value: String,
             client: AmazonEC2) {
  ...
  def hasTag(instance: ASGInstance,
             key: String,
             value: String,
             client: AmazonEC2): Boolean = {
    describe(instance, client).getTags.asScala exists { tag =>
      tag.getKey == key && tag.getValue == value
    }
  }

  def describe(instance: ASGInstance, client: AmazonEC2) =
    client
      .describeInstances(
  ...
  def apply(instance: ASGInstance, client: AmazonEC2) =
...
object CloudFormation extends AWS {
  ...
  def validateTemplate(template: Template, client: AmazonCloudFormation) = {
    val request = template match {
      case TemplateBody(body) =>
        new ValidateTemplateRequest().withTemplateBody(body)
      case TemplateUrl(url) =>
        new ValidateTemplateRequest().withTemplateURL(url)
    }
    client.validateTemplate(request)
  }

  def updateStack(name: String,
                  template: Template,
  ...
  def updateStackParams(name: String,
                        parameters: Map[String, ParameterValue],
                        client: AmazonCloudFormation) =
    client.updateStack(
  ...
  def createStack(reporter: DeployReporter,
    ...
    maybeTags.foreach { tags =>
      val sdkTags = tags.map {
        case (key, value) => new CfnTag().withKey(key).withValue(value)
      }
  ...
  def findStackByTags(tags: Map[String, String],
                      reporter: DeployReporter,
                      client: AmazonCloudFormation): Option[AmazonStack] = {

    def tagsMatch(stack: AmazonStack): Boolean =
      tags.forall {
        case (key, value) =>
          stack.getTags.asScala.exists(t =>
            t.getKey == key && t.getValue == value)
      }

    @tailrec
    def recur(nextToken: Option[String] = None,
              existingStacks: List[AmazonStack] = Nil): List[AmazonStack] = {
    ...
    recur() match {
      case cfnStack :: Nil =>
        reporter.verbose(
          s"Found stack ${cfnStack.getStackName} (${cfnStack.getStackId})")
        Some(cfnStack)
      case Nil =>
        None
      case multipleStacks =>
        reporter.fail(
          s"More than one cloudformation stack match for $tags (matched ${multipleStacks.map(_.getStackName).mkString(", ")}). Failing fast since this may be non-deterministic.")
    }
  }
}

object STS extends AWS {
  def makeSTSclient(keyRing: KeyRing,
                    region: Region): AWSSecurityTokenService = {
    AWSSecurityTokenServiceClientBuilder
      .standard()
      .withCredentials(provider(keyRing))
      .withClientConfiguration(clientConfiguration)
...
trait AWS extends Loggable {
  ...
  def provider(keyRing: KeyRing): AWSCredentialsProvider =
  ...
  class LoggingRetryCondition extends SDKDefaultRetryCondition {
    private def exceptionInfo(e: Throwable): String = {
      s"${e.getClass.getName} ${e.getMessage} Cause: ${Option(e.getCause)
        .map(e => exceptionInfo(e))}"
    }

    override def shouldRetry(originalRequest: AmazonWebServiceRequest,
                             exception: AmazonClientException,
                             retriesAttempted: Int): Boolean = {
      val willRetry =
        super.shouldRetry(originalRequest, exception, retriesAttempted)
      if (willRetry) {
        logger.warn(
          s"AWS SDK retry $retriesAttempted: ${Option(originalRequest).map(
            _.getClass.getName)} threw ${exceptionInfo(exception)}")
      } else {
        logger.warn(s"Encountered fatal exception during AWS API call",
                    exception)
  ...
  val clientConfigurationNoRetry = new ClientConfiguration()
