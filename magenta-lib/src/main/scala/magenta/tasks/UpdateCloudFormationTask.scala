...
object UpdateCloudFormationTask {
  ...
  object LookupByName {
    def apply(stack: Stack,
              stage: Stage,
              cfnStackName: String,
              prependStack: Boolean,
              appendStage: Boolean): LookupByName = {
  ...
  object LookupByTags {
    def apply(pkg: DeploymentPackage,
              target: DeployTarget,
              reporter: DeployReporter): LookupByTags = {
  ...
  def combineParameters(
      ...
      parameters: Map[String, String]): Map[String, ParameterValue] = {
    def addParametersIfInTemplate(params: Map[String, ParameterValue])(
        nameValues: Iterable[(String, String)])
      : Map[String, ParameterValue] = {
      nameValues.foldLeft(params) {
        case (completeParams, (name, value))
            if templateParameters.exists(_.key == name) =>
          completeParams + (name -> SpecifiedValue(value))
        case (completeParams, _) => completeParams
      }
    }

    val requiredParams: Map[String, ParameterValue] = templateParameters
      .filterNot(_.default)
      .map(_.key -> UseExistingValue)
      .toMap
    val userAndDefaultParams = requiredParams ++ parameters.mapValues(
      SpecifiedValue.apply)
  ...
  def nameToCallNewStack(strategy: CloudFormationStackLookupStrategy): String = {
    ...
    strategy match {
      ...
      case LookupByTags(tags) =>
        val orderedTags = tags.toList.sortBy {
          case (key, value) =>
...
case class UpdateCloudFormationTask(
    ...
    alwaysUploadToS3: Boolean)(implicit val keyRing: KeyRing,
  ...
  override def execute(reporter: DeployReporter, stopFlag: => Boolean) =
    if (!stopFlag) {
      ...
      val nameToCallStack = UpdateCloudFormationTask.nameToCallNewStack(
        cloudFormationStackLookupStrategy)

      val template = processTemplate(nameToCallStack,
                                     templateString,
                                     s3Client,
                                     stsClient,
                                     region,
      ...
      val templateParameters = CloudFormation
        ...
        .getParameters
        .map(tp =>
          TemplateParameter(tp.getParameterKey,
                            Option(tp.getDefaultValue).isDefined))

      val resolvedAmiParameters: Map[String, String] =
        amiParameterMap.flatMap {
          case (name, tags) =>
      ...
      val parameters: Map[String, ParameterValue] =
        combineParameters(stack,
                          stage,
                          templateParameters,
                          userParameters ++ resolvedAmiParameters)
      ...
      maybeCfStack match {
        case Some(cloudFormationStackName) =>
          try {
            CloudFormation.updateStack(cloudFormationStackName.getStackName,
                                       template,
                                       parameters,
                                       cfnClient)
          } catch {
            case ase: AmazonServiceException
                if ase.getMessage contains "No updates are to be performed." =>
              reporter.info(
                "Cloudformation update has no changes to template or parameters")
            case ase: AmazonServiceException
                if ase.getMessage contains "Template format error: JSON not well-formed" =>
              reporter.info(
                s"Cloudformation update failed with the following template content:\n$templateString")
              throw ase
          }
        case None =>
          if (createStackIfAbsent) {
            val stackTags =
              PartialFunction.condOpt(cloudFormationStackLookupStrategy) {
                case LookupByTags(tags) => tags
              }
            reporter.info(
              s"Stack $cloudFormationStackLookupStrategy doesn't exist. Creating stack using name $nameToCallStack.")
            CloudFormation.createStack(reporter,
                                       nameToCallStack,
                                       stackTags,
                                       template,
                                       parameters,
            ...
            reporter.fail(
              s"Stack $cloudFormationStackLookupStrategy doesn't exist and createStackIfAbsent is false")
          }
      }
    }

  def description =
    s"Updating CloudFormation stack $cloudFormationStackLookupStrategy with ${templatePath.fileName}"
  ...
  override def execute(reporter: DeployReporter, stopFlag: => Boolean) =
    if (!stopFlag) {
      ...
      val resolvedAmiParameters: Map[String, ParameterValue] =
        amiParameterMap.flatMap {
          case (parameterName, amiTags) =>
            if (!cfStack.getParameters.exists(
                  _.getParameterKey == parameterName)) {
              reporter.fail(
                s"stack ${cfStack.getStackName} does not have an $parameterName parameter to update")
            }

            val currentAmi = cfStack.getParameters
            ...
            maybeNewAmi match {
              ...
              case None =>
                ...
                reporter.fail(
                  s"Failed to resolve AMI for ${cfStack.getStackName} parameter $parameterName with tags: $tagsStr")
            }
        }
      ...
      if (resolvedAmiParameters.nonEmpty) {
        ...
        CloudFormation.updateStackParams(cfStack.getStackName,
                                         newParameters,
                                         cfnClient)
      } else {
        reporter.info(
...
case class CheckUpdateEventsTask(
    ...
    val stackName = stackLookupStrategy match {
      ...
      case strategy @ LookupByTags(tags) =>
        val stack = CloudFormation
          .findStackByTags(tags, reporter, cfnClient)
          .getOrElse(
            reporter.fail(s"Could not find CloudFormation stack $strategy"))
        stack.getStackName
    }
      ...
      lastSeenEvent match {
        case None =>
          events.find(updateStart(stackName)) foreach (e => {
            val age = new Duration(new DateTime(e.getTimestamp),
                                   new DateTime()).getStandardSeconds
            if (age > 30) {
              reporter.verbose(
        ...
        case Some(event) =>
          val newEvents =
            events.takeWhile(_.getTimestamp.after(event.getTimestamp))
  ...
  object StackEvent {
    def reportEvent(reporter: DeployReporter, e: StackEvent): Unit = {
      ...
      if (e.getResourceStatusReason != null)
        reporter.verbose(e.getResourceStatusReason)
    }
