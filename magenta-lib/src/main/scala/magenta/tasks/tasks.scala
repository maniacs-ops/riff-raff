...
case class S3Upload(
  ...
  clientFactory: (KeyRing, Region, ClientConfiguration) => AmazonS3 =
    S3.makeS3client)
    extends Task
    with Loggable {
  ...
  lazy val totalSize = objectMappings.map {
    case (source, target) => source.size
  }.sum
  ...
  def description: String =
    s"Upload ${fileString(objectMappings.size)} to S3 bucket $bucket"
    ...
    requests.zipWithIndex.par foreach {
      case (req, index) =>
        ...
        index match {
          ...
          case 10 =>
            reporter.verbose(
              s"Not logging details for the remaining ${fileString(
                objectMappings.size - 10)}")
          case _ =>
        }
        retryOnException(S3.clientConfiguration) {
          val inputStream = artifactClient
            .getObject(req.source.bucket, req.source.key)
            .getObjectContent
  ...
  private def resolveMappings(
      path: S3Location,
      targetKey: String,
      targetBucket: String): Seq[(S3Object, S3Path)] = {
    path.listAll()(artifactClient).map { obj =>
      obj -> S3Path(targetBucket,
...
object S3Upload {
  ...
  def prefixGenerator(stack: Option[Stack] = None,
    ...
    (stack.flatMap(_.nameOption) :: stage.map(_.name) :: packageName :: Nil).flatten
      .mkString("/")
  }
  def prefixGenerator(stack: Stack,
                      stage: Stage,
...
case class PutReq(source: S3Object,
  ...
  def toAwsRequest(inputStream: InputStream): PutObjectRequest = {
    ...
    val req =
      new PutObjectRequest(target.bucket, target.key, inputStream, metaData)
...
case class ChangeSwitch(host: Host,
  ...
  override def execute(reporter: DeployReporter, stopFlag: => Boolean) = {
    reporter.verbose(
      s"Changing $switchName to $desiredStateName using $switchboardUrl")
...
case class UpdateLambda(s3Path: S3Path, functionName: String, region: Region)(
    implicit val keyRing: KeyRing,
    artifactClient: AmazonS3)
    extends Task {
  ...
  override def execute(reporter: DeployReporter, stopFlag: => Boolean) {
    ...
    val inputStream =
      artifactClient.getObject(s3Path.bucket, s3Path.key).getObjectContent
    val buffer = try {
    ...
    client.updateFunctionCode(
      Lambda.lambdaUpdateFunctionCodeRequest(functionName, buffer))
    reporter.verbose(s"Finished update $functionName Lambda")
  }

}

case class UpdateS3Lambda(functionName: String,
                          s3Bucket: String,
                          s3Key: String,
