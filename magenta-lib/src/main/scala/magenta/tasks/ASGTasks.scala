...
case class CheckGroupSize(pkg: DeploymentPackage,
                          stage: Stage,
                          stack: Stack,
  ...
  override def execute(asg: AutoScalingGroup,
    ...
    reporter.verbose(
      s"ASG desired = ${asg.getDesiredCapacity}; ASG max = ${asg.getMaxSize}; Target = $doubleCapacity")
...
case class TagCurrentInstancesWithTerminationTag(
  ...
  override def execute(asg: AutoScalingGroup,
    ...
    if (asg.getInstances.nonEmpty) {
      ...
      reporter.verbose(
        s"Tagging ${asg.getInstances.toList.map(_.getInstanceId).mkString(", ")}")
  ...
  lazy val description =
    "Tag existing instances of the auto-scaling group for termination"
}

case class DoubleSize(pkg: DeploymentPackage,
                      stage: Stage,
                      stack: Stack,
                      region: Region)(implicit val keyRing: KeyRing)
    extends ASGTask {
...
sealed abstract class Pause(durationMillis: Long)(
...
case class HealthcheckGrace(pkg: DeploymentPackage,
                            stage: Stage,
                            stack: Stack,
                            region: Region,
                            durationMillis: Long)(implicit keyRing: KeyRing)
    extends Pause(durationMillis) {
  def description: String =
    s"Wait extra ${durationMillis}ms to let Load Balancer report correctly"
}

case class WarmupGrace(pkg: DeploymentPackage,
                       stage: Stage,
                       stack: Stack,
                       region: Region,
                       durationMillis: Long)(implicit keyRing: KeyRing)
...
case class TerminationGrace(pkg: DeploymentPackage,
                            stage: Stage,
                            stack: Stack,
                            region: Region,
                            durationMillis: Long)(implicit keyRing: KeyRing)
...
case class CheckForStabilization(pkg: DeploymentPackage,
                                 stage: Stage,
                                 stack: Stack,
                                 region: Region)(implicit val keyRing: KeyRing)
    extends ASGTask {
...
case class WaitForStabilization(pkg: DeploymentPackage,
                                stage: Stage,
                                stack: Stack,
                                duration: Long,
                                region: Region)(implicit val keyRing: KeyRing)
  ...
  override def execute(asg: AutoScalingGroup,
    ...
    def isRateExceeded(e: AmazonServiceException) =
      e.getStatusCode == 400 && e.getErrorCode == "Throttling"
  }
...
case class CullInstancesWithTerminationTag(
  ...
  override def execute(asg: AutoScalingGroup,
    ...
    val instancesToKill = asg.getInstances.filter(instance =>
      EC2.hasTag(instance, "Magenta", "Terminate", ec2Client))
    val orderedInstancesToKill =
      instancesToKill.transposeBy(_.getAvailabilityZone)
    reporter.verbose(
      s"Culling instances: ${orderedInstancesToKill.map(_.getInstanceId).mkString(", ")}")
    orderedInstancesToKill.foreach(instance =>
      ASG.cull(asg, instance, asgClient, elbClient))
  }

  lazy val description =
    "Terminate instances with the termination tag for this deploy"
}
...
case class SuspendAlarmNotifications(
  ...
  lazy val description =
    "Suspending Alarm Notifications - group will no longer scale on any configured alarms"
}

case class ResumeAlarmNotifications(
    pkg: DeploymentPackage,
    stage: Stage,
    stack: Stack,
    region: Region)(implicit val keyRing: KeyRing)
  ...
  lazy val description =
    "Resuming Alarm Notifications - group will scale on any configured alarms"
}
