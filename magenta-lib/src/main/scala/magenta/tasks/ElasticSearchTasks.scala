...
case class WaitForElasticSearchClusterGreen(
  ...
  override def execute(asg: AutoScalingGroup,
    ...
    val instance = EC2(asg.getInstances.headOption.getOrElse {
      throw new IllegalArgumentException(
        "Auto-scaling group: %s had no instances" format (asg))
    }, ec2Client)
    val node = ElasticSearchNode(instance.getPublicDnsName)
    check(reporter, stopFlag) {
      node.inHealthyClusterOfSize(
        ASG.refresh(asg, asgClient).getDesiredCapacity)
    }
  }
}

case class CullElasticSearchInstancesWithTerminationTag(
    pkg: DeploymentPackage,
    stage: Stage,
    stack: Stack,
    duration: Long,
  ...
  override def execute(asg: AutoScalingGroup,
    ...
    val newNode = asg.getInstances
      .filterNot(EC2.hasTag(_, "Magenta", "Terminate", ec2Client))
      .head
    ...
    def cullInstance(instance: Instance) {
      ...
      check(reporter, stopFlag) {
        newESNode.inHealthyClusterOfSize(
          ASG.refresh(asg, asgClient).getDesiredCapacity)
      }
      if (!stopFlag) {
        node.shutdown()
        check(reporter, stopFlag) {
          newESNode.inHealthyClusterOfSize(
            ASG.refresh(asg, asgClient).getDesiredCapacity - 1)
        }
      }
...
case class ElasticSearchNode(address: String) {
  ...
  def shutdown() =
    http(
      (:/(address, 9200) / "_cluster" / "nodes" / "_local" / "_shutdown").POST >|)
}
