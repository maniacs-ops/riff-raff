...
object `package` extends Loggable {
  def transpose[A](xs: Seq[Seq[A]]): Seq[Seq[A]] =
  ...
  @tailrec
  def retryOnException[T](config: ClientConfiguration,
                          currentAttempt: Int = 1)(f: => T): T = {
    ...
    def shouldRetryFor(ace: AmazonClientException) =
      policy.getRetryCondition.shouldRetry(null, ace, currentAttempt)

    Try(f) match {
      case Success(result) => result
      case Failure(exception: AmazonClientException)
        ...
        val delay = policy.getBackoffStrategy.delayBeforeNextRetry(
          null,
          exception,
          currentAttempt)
