...
object Lambda extends DeploymentType {
  ...
  val fileNameParam =
    Param[String]("fileName", "The name of the archive of the function")
  ...
  def lambdaToProcess(pkg: DeploymentPackage,
                      target: DeployTarget,
                      reporter: DeployReporter): List[LambdaFunction] = {
    ...
    val regions: List[Region] = regionsOption
      .map(_.filter(regionExists(_)(reporter)).map(Region))
      .getOrElse(List(target.region))
    val stage = target.parameters.stage.name

    (functionNamesParam.get(pkg),
     functionsParam.get(pkg),
     prefixStackParam(pkg, target, reporter)) match {
      case (Some(functionNames), None, prefixStack) =>
        val stackNamePrefix =
          target.stack.nameOption.filter(_ => prefixStack).getOrElse("")
        for {
          name <- functionNames
          region <- regions
        } yield
          LambdaFunction(s"$stackNamePrefix$name$stage",
      ...
      case _ =>
        reporter.fail(
          "Must specify one of 'functions' or 'functionNames' parameters")
    }
  }
  ...
  private def regionExists(name: String)(reporter: DeployReporter): Boolean = {
    ...
    if (!exists) {
      reporter.warning(
        s"$name is not a recognised AWS region and has been ignored.")
    }
    exists
  }

  def makeS3Key(stack: Stack,
                params: DeployParameters,
                pkg: DeploymentPackage,
                fileName: String): String = {
    List(stack.nameOption,
         Some(params.stage.name),
         Some(pkg.name),
         Some(fileName)).flatten.mkString("/")
  }
  ...
  val updateLambda = Action(
    ...
    lambdaToProcess(pkg, target, resources.reporter).flatMap {
      case LambdaFunctionFromZip(functionName, fileName, region) =>
        Some(
          UpdateLambda(S3Path(pkg.s3Package, fileName), functionName, region))
...
case class LambdaFunctionFromS3(functionName: String,
                                fileName: String,
                                region: Region,
                                s3Bucket: String)
    extends LambdaFunction
case class LambdaFunctionFromZip(functionName: String,
                                 fileName: String,
                                 region: Region)
    extends LambdaFunction
...
object LambdaFunction {
  def apply(functionName: String,
            fileName: String,
            region: Region,
            s3Bucket: Option[String] = None): LambdaFunction = {
