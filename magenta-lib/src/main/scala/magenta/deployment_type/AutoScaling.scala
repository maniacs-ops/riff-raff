...
object AutoScaling extends DeploymentType {
  ...
  val secondsToWait =
    Param("secondsToWait",
          "Number of seconds to wait for instances to enter service").default(
      15 * 60)
  val healthcheckGrace =
    Param("healthcheckGrace",
          "Number of seconds to wait for the AWS api to stabilise").default(20)
  ...
  val prefixStage =
    Param[Boolean]("prefixStage",
                   documentation =
                     "Whether to prefix `stage` to the S3 location")
      .default(true)
  val prefixPackage =
    Param[Boolean]("prefixPackage",
                   documentation =
                     "Whether to prefix `package` to the S3 location")
      .default(true)
  val prefixStack =
    Param[Boolean]("prefixStack",
                   documentation =
                     "Whether to prefix `stack` to the S3 location")
      .default(true)
  ...
  val deploy = Action(
    ...
    List(
      ...
      HealthcheckGrace(pkg,
                       parameters.stage,
                       stack,
                       target.region,
                       healthcheckGrace(pkg, target, reporter) * 1000),
      WaitForStabilization(pkg,
                           parameters.stage,
                           stack,
      ...
      WaitForStabilization(pkg,
                           ...
                           secondsToWait(pkg, target, reporter) * 1000,
                           target.region),
      CullInstancesWithTerminationTag(pkg,
                                      parameters.stage,
                                      stack,
                                      target.region),
      TerminationGrace(pkg,
                       parameters.stage,
                       stack,
                       target.region,
