...
case class Param[T](
    ...
    defaultValueFromContext: Option[(DeploymentPackage,
                                     DeployTarget) => Either[String, T]] = None
  ...
  def apply(pkg: DeploymentPackage,
            target: DeployTarget,
            reporter: DeployReporter)(implicit reads: Reads[T],
  ...
  def defaultFromContext(
      defaultFromContext: (DeploymentPackage,
                           DeployTarget) => Either[String, T]) = {
