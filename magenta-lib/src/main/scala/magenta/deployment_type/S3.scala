  ...
  ) { (pkg, resources, target) =>
    {
      def resourceLookupFor(resource: Param[String]): Option[Datum] = {
        resource.get(pkg).flatMap { resourceName =>
          ...
          val datumOpt = dataLookup
            .datum(resourceName, app, target.parameters.stage, target.stack)
          if (datumOpt.isEmpty) {
      ...
      val bucketName = bucket.get(pkg) getOrElse {
        ...
        assert(
          data.isDefined,
          s"Cannot find resource value for ${bucketResource(
            pkg,
            target,
            reporter)} (${pkg.apps.head} in ${target.parameters.stage.name})")
        data.get.value
      }
