...
case class RecipeTasks(recipe: Recipe, tasks: List[Task]) {
  lazy val hosts =
    tasks.flatMap(_.taskHost).map(_.copy(connectAs = None)).distinct
  lazy val recipeName = recipe.name
}

case class RecipeTasksNode(recipeTasks: RecipeTasks,
                           children: List[RecipeTasksNode]) {
  def toList: List[RecipeTasks] =
    children.flatMap(_.toList) ++ List(recipeTasks)

  def toGraph(name: String): Graph[DeploymentTasks] = {
    val thisGraph: Graph[DeploymentTasks] =
      DeploymentGraph(recipeTasks.tasks, s"$name (${recipeTasks.recipeName})")

    val childGraph: Graph[DeploymentTasks] =
      children
        .map(_.toGraph(name))
        .reduceLeftOption(_ joinParallel _)
...
object Resolver {
  ...
  def resolve(project: Project,
    ...
    resolveStacks(project, parameters, resources.reporter)
      .map { stack =>
        val stackTasks =
          resolveStack(project, parameters, resources, stack, region).flatMap(
            _.tasks)
        DeploymentGraph(
          stackTasks,
  ...
  def resolveStack(project: Project,
    ...
    def resolveTree(recipeName: String,
                    resources: DeploymentResources,
                    target: DeployTarget): RecipeTasksNode = {
      val recipe = project.recipes.getOrElse(
        recipeName,
        sys.error(
          s"Recipe '$recipeName' doesn't exist in your deploy.json file"))
    ...
    def resolveRecipe(recipe: Recipe,
                      resources: DeploymentResources,
                      target: DeployTarget): RecipeTasks = {
      val tasks = for {
    ...
    for {
      tasks <- {
        ...
        val resolvedTree =
          resolveTree(parameters.recipe.name, resources, target)
        resolvedTree.toList.distinct
      }
    } yield tasks
  }

  def resolveStacks(project: Project,
                    parameters: DeployParameters,
                    reporter: DeployReporter): Seq[Stack] = {
