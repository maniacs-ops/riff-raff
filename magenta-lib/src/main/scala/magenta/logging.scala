...
case class ThrowableDetail(name: String,
                           message: String,
                           stackTrace: String,
                           cause: Option[ThrowableDetail] = None)
object ThrowableDetail {
  implicit def Throwable2ThrowableDetail(t: Throwable): ThrowableDetail =
    ThrowableDetail(t)
...
case class TaskDetail(name: String,
                      description: String,
                      verbose: String,
                      taskHosts: List[Host]) {
...
object TaskDetail {
  ...
  implicit def TaskList2TaskDetailList(tl: List[Task]): List[TaskDetail] =
    tl.map(TaskDetail(_)).toList
...
case class DeployReporter(messageStack: List[Message],
  ...
  def taskList(tasks: List[Task]) {
    DeployReporter.send(this, TaskList(tasks))
  }
  ...
  def commandOutput(message: String) {
    DeployReporter.send(this, CommandOutput(message))
  }
  def commandError(message: String) {
    DeployReporter.send(this, CommandError(message))
  }
...
object DeployReporter {
  ...
  private def send(reporter: DeployReporter,
                   message: Message,
    ...
    MagentaMetrics.MessageBrokerMessages.measure {
      messageSubject.onNext(
        MessageWrapper(reporter.messageContext, messageUUID, stack))
    }
  }

  // get the genesis reporter that will be the root of all others
  def rootReporterFor(uuid: UUID,
                      parameters: DeployParameters,
                      publishMessages: Boolean = true): DeployReporter = {
    DeployReporter(Nil,
                   MessageContext(uuid, parameters, None),
                   publishMessages = publishMessages)
  }

  def startDeployContext(reporter: DeployReporter) =
    pushContext(Deploy(reporter.messageContext.parameters), reporter)

  // create a new reporter with the given message at the top of the stack - sends the StartContext event
  def pushContext(message: Message,
                  currentReporter: DeployReporter): DeployReporter = {
  ...
  def finishContext(currentReporter: DeployReporter): Option[DeployReporter] = {
    currentReporter.messageStack.headOption.foreach(msg =>
      send(currentReporter, FinishContext(msg)))
  ...
  def failContext(currentReporter: DeployReporter): Option[DeployReporter] = {
    currentReporter.messageStack.headOption.foreach(msg =>
      send(currentReporter, FailContext(msg)))
    currentReporter.previousReporter
  }
  def failContext(currentReporter: DeployReporter,
                  message: String,
                  reason: Throwable) {
  ...
  def withFailureHandling[T](reporter: DeployReporter)(
  ...
  private def sendContext[T](
      currentReporter: DeployReporter,
      message: Message)(block: (DeployReporter) => T): T = {
  ...
  private def failException(reporter: DeployReporter,
                            message: String,
                            e: Option[Throwable] = None): FailException = {
  ...
  private def failException(reporter: DeployReporter,
                            message: String,
                            e: Throwable): FailException = {
    failException(reporter, message, Some(e))
  }
}

case class MessageContext(deployId: UUID,
                          parameters: DeployParameters,
                          parentId: Option[UUID])
case class MessageWrapper(context: MessageContext,
                          messageId: UUID,
                          stack: MessageStack)
...
case class MessageStack(messages: List[Message],
  ...
  lazy val deployParameters: Option[DeployParameters] = {
    messages
      .filter(_.deployParameters.isDefined)
      .lastOption
      .flatMap(_.deployParameters)
  }
}

class FailException(val message: String, val throwable: Throwable = null)
...
sealed trait ContextMessage extends Message {
  ...
  override def deployParameters: Option[DeployParameters] =
    originalMessage.deployParameters
}
...
case class TaskRun(task: TaskDetail) extends Message {
  lazy val text = s"task ${task.fullDescription}"
}
