...
object MessageState {
  ...
  def apply(message: StartContext,
            end: ContextMessage,
            time: DateTime,
            id: UUID): MessageState = {
    end match {
      case finish: FinishContext =>
        FinishMessageState(message, finish, time, id)
...
case class SimpleMessageState(message: Message,
                              time: DateTime,
                              messageId: UUID)
...
case class StartMessageState(startContext: StartContext,
                             time: DateTime,
                             messageId: UUID)
    extends MessageState {
...
case class FinishMessageState(startContext: StartContext,
...
case class FailMessageState(startContext: StartContext,
                            fail: FailContext,
                            time: DateTime,
                            messageId: UUID)
    extends MessageState {
...
trait DeployReport {
  ...
  def failureMessage: Option[Fail] =
    allMessages.map(_.message).collect { case fail: Fail => fail }.headOption
...
object DeployReport {
  private def wrapperToTree(node: MessageWrapper,
                            all: List[MessageWrapper]): DeployReportTree = {
    val allChildren =
      all.filter(_.context.parentId.exists(_ == node.messageId))
    ...
    val messageState = endOption match {
      case Some(end) =>
        MessageState(node.stack.top.asInstanceOf[StartContext],
                     end,
                     node.stack.time,
                     node.messageId)
      case None =>
        MessageState(node.stack.top, node.stack.time, node.messageId)
    }
...
case class DeployReportTree(messageState: MessageState,
