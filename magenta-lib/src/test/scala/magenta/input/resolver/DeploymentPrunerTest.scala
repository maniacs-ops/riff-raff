...
class DeploymentPrunerTest extends FlatSpec with Matchers {
  ...
  val testDeployment = Deployment("testName",
                                  "testType",
                                  NEL.of("testStack"),
                                  NEL.of("testRegion"),
                                  NEL.of("testAction"),
  ...
  "StackAndRegion pruner" should "return None when either the region or stack isn't specified" in {
    testStackAndRegionPruner(
      testDeployment.copy(stacks = NEL.of("differentStack"))) shouldBe None
    testStackAndRegionPruner(
      testDeployment.copy(regions = NEL.of("differentRegion"))) shouldBe None
    testStackAndRegionPruner(
      testDeployment.copy(stacks = NEL.of("differentStack"),
                          regions = NEL.of("differentRegion"))) shouldBe None
  }
