...
class JsonReaderTest extends FlatSpec with Matchers {
  ...
  "json parser" should "parse json and resolve links" in {
    val parsedProject =
      JsonReader.buildProject(deployJsonExample,
                              S3JsonArtifact("artifact-bucket", "test/123"),
                              deploymentTypes)

    parsedProject.applications should be(
      Set(App("index-builder"), App("api"), App("solr")))
    ...
    parsedProject.packages("index-builder") shouldBe
      DeploymentPackage("index-builder",
                        ...
                        Map.empty,
                        "autoscaling",
                        S3Path("artifact-bucket",
                               "test/123/packages/index-builder/"),
                        true,
                        deploymentTypes)
    parsedProject.packages("api") shouldBe
      DeploymentPackage(
        ...
        Seq(App("api")),
        Map(
          "healthcheck_paths" -> Json.arr("/api/index.json",
                                          "/api/search.json")),
        "autoscaling",
    ...
    recipes("all") should be(
      Recipe("all", Nil, List("index-build-only", "api-only")))
...
""")
  ...
  "json parser" should "infer a single app if none specified" in {
    val parsedProject =
      JsonReader.buildProject(minimalExample,
                              S3JsonArtifact("artifact-bucket", "test/123"),
                              deploymentTypes)
...
""")
  ...
  "json parser" should "infer a default recipe that deploys all packages" in {
    val parsedProject =
      JsonReader.buildProject(twoPackageExample,
                              S3JsonArtifact("artifact-bucket", "test/123"),
                              deploymentTypes)
    ...
    recipes("default") should be(
      Recipe("default",
             deploymentSteps = parsedProject.packages.values
               .map(_.mkDeploymentStep("deploy"))))
  }
  ...
  "json parser" should "default to using the package name for the file name" in {
    ...
    parsedProject.packages("dinky").s3Package should be(
      S3Path("artifact-bucket", "test/123/packages/dinky/"))
  }
...
""")
  ...
  "json parser" should "use override file name if specified" in {
    val parsedProject =
      JsonReader.buildProject(withExplicitFileName,
                              S3JsonArtifact("artifact-bucket", "test/123"),
                              deploymentTypes)

    parsedProject.packages("dinky").s3Package should be(
      S3Path("artifact-bucket", "test/123/packages/awkward/"))
  }
