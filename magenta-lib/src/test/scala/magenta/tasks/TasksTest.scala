...
class TasksTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  "PutRec" should "create an upload request with correct permissions" in {
    ...
    awsRequestWithoutAcl.getCannedAcl should be(
  ...
  it should "create an upload request with cache control and content type" in {
    ...
    Option(awsRequest.getMetadata.getContentType) should be(
      Some("application/json"))
  }
  ...
  it should "use a default mime type from S3" in {
    ...
    awsRequestTwo.getMetadata.getContentType should be(
      "application/octet-stream")
    awsRequestThree.getMetadata.getContentType should be(
      "application/x-javascript")
  }

  "S3Upload" should "upload a single file to S3" in {
    val artifactClient = mock[AmazonS3Client]
    val objectResult = mockListObjects(
      List(MagentaS3Object("artifact-bucket", "foo/bar/the-jar.jar", 31)))
    ...
    val task = new S3Upload(Region("eu-west-1"),
                            "destination-bucket",
                            Seq(fileToUpload -> "keyPrefix/the-jar.jar"))(
      fakeKeyRing,
      artifactClient,
  ...
  it should "upload a directory to S3" in {
    ...
    when(artifactClient.listObjectsV2(any[ListObjectsV2Request]))
      .thenReturn(objectResult)
    when(artifactClient.getObject(any[String], any[String]))
      .thenReturn(mockObject("Some content for this S3 object"))
  ...
  it should "upload a directory to S3 with no prefix" in {
    ...
    when(s3Client.putObject(any[PutObjectRequest])).thenReturn(putObjectResult)
    val task =
      new S3Upload(Region("eu-west-1"), "bucket", Seq(packageRoot -> ""))(
        fakeKeyRing,
        artifactClient,
        clientFactory(s3Client))
    task.execute(reporter)
  ...
  it should "use different cache control" in {
    ...
    val patternValues =
      List(PatternValue("^keyPrefix/sub/", "public; max-age=3600"),
  ...
  it should "use overridden mime type" in {
    ...
    val task = new S3Upload(
      ...
      "bucket",
      Seq(packageRoot -> ""),
      extensionToMimeType = mimeTypes)(fakeKeyRing, artifactClient)
  ...
  def mockObject(content: String): S3Object = {
    ...
    mockedObject.setObjectContent(
      new ByteArrayInputStream(content.getBytes("UTF-8")))
    mockedObject
  }

  def clientFactory(
      client: AmazonS3): (KeyRing, Region, ClientConfiguration) => AmazonS3 = {
    (_, _, _) =>
      client
  }
