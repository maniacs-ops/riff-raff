...
class ASGTasksTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  val deploymentTypes = Seq(
    stubDeploymentType(name = "testDeploymentType",
  ...
  it should "double the size of the autoscaling group" in {
    ...
    val task =
      new DoubleSize(p, Stage("PROD"), UnnamedStack, Region("eu-west-1"))
  ...
  it should "fail if you do not have the capacity to deploy" in {
    ...
    val task =
      new CheckGroupSize(p, Stage("PROD"), UnnamedStack, Region("eu-west-1"))
