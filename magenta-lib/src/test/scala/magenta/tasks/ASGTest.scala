...
import com.amazonaws.services.elasticloadbalancing.{
  AmazonElasticLoadBalancingClient => ClassicELBClient
}
import com.amazonaws.services.elasticloadbalancingv2.{
  AmazonElasticLoadBalancingClient => ApplicationELBClient
}
...
class ASGTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  it should "find the matching auto-scaling group with App tagging" in {
    ...
    when(
      asgClientMock.describeAutoScalingGroups(
        any[DescribeAutoScalingGroupsRequest])) thenReturn
    ...
    val p = DeploymentPackage("example",
                              ...
                              true)
    ASG.groupForAppAndStage(p,
                            Stage("PROD"),
                            UnnamedStack,
                            asgClientMock,
                            reporter) should be(desiredGroup)
  }
  ...
  it should "find the matching auto-scaling group with Stack and App tags" in {
    ...
    val desiredGroup = AutoScalingGroup("Stack" -> "contentapi",
                                        "App" -> "logcabin",
                                        "Stage" -> "PROD")
    ...
    when(
      ...
      new DescribeAutoScalingGroupsResult().withAutoScalingGroups(List(
        ...
        AutoScalingGroup("Role" -> "other", "Stage" -> "PROD"),
        AutoScalingGroup("Role" -> "example", "Stage" -> "TEST"),
        AutoScalingGroup("Stack" -> "contentapi",
                         "App" -> "logcabin",
                         "Stage" -> "TEST"),
        AutoScalingGroup("Stack" -> "contentapi",
                         "App" -> "elasticsearch",
                         "Stage" -> "PROD"),
        AutoScalingGroup("Stack" -> "monkey",
                         "App" -> "logcabin",
                         "Stage" -> "PROD")
      ))
  ...
  it should "wait for instances in ELB to stabilise if there is one" in {
    ...
    when(
      ...
      )).thenReturn(new DescribeInstanceHealthResult()
      .withInstanceStates(new InstanceState().withState("")))

    ASG.isStabilized(group,
                     asgClientMock,
                     ELB.Client(classicELBClient, appELBClient)) shouldBe
    ...
    when(
      ...
      .withInstanceStates(new InstanceState().withState("InService")))

    ASG.isStabilized(
      group,
      asgClientMock,
      ELB.Client(classicELBClient, appELBClient)) shouldBe Right(())
  }
  ...
  it should "wait for instances in an ELB target group to be bealthy" in {
    ...
    when(
      ...
      new DescribeTargetHealthResult().withTargetHealthDescriptions(
        new TargetHealthDescription().withTargetHealth(
          new TargetHealth().withState(TargetHealthStateEnum.Unhealthy))))
    ...
    when(
      ...
      new DescribeTargetHealthResult().withTargetHealthDescriptions(
        new TargetHealthDescription().withTargetHealth(
          new TargetHealth().withState(TargetHealthStateEnum.Healthy))))
  ...
  it should "just check ASG health for stability if there is no ELB" in {
    ...
    val updatedGroup = AutoScalingGroup("Role" -> "example", "Stage" -> "PROD")
      .withDesiredCapacity(1)
      .withInstances(
        new ASGInstance().withLifecycleState(LifecycleState.InService))
  ...
  it should "find the first matching auto-scaling group with Stack and App tags, on the second page of results" in {
    ...
    val secondRequest =
      new DescribeAutoScalingGroupsRequest().withNextToken("someToken")
