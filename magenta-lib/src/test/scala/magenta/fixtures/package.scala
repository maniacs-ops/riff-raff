...
package object fixtures {
  ...
  def project(recipe: Recipe, stacks: Stack*) =
    Project(Map.empty, Map(recipe.name -> recipe), defaultStacks = stacks)

  def stubPackage(deploymentType: DeploymentType) =
    DeploymentPackage("stub project",
                      Seq(app1),
                      Map(),
                      "stub-package-type",
                      null,
  ...
  def stubLookup(hostsSeq: Seq[Host] = Nil,
                 resourceData: Map[String, Seq[Datum]] = Map.empty): Lookup = {
    new Lookup {
          ...
          stack match {
            ...
            case NamedStack(stackName) =>
              matchingList.filter(_.stack.isDefined).find { data =>
                data.stackRegex
                  .exists(_.findFirstMatchIn(stackName).isDefined) &&
                data.appRegex.findFirstMatchIn(app.name).isDefined &&
                data.stageRegex.findFirstMatchIn(stage.name).isDefined
              }
          }
      ...
      def credentials(stage: Stage,
                      apps: Set[App]): Map[String, ApiCredentials] = ???
      ...
      def getLatestAmi(region: String)(
          tags: Map[String, String]): Option[String] = ???
    }
  }
