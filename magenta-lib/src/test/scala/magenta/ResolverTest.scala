...
class ResolverTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  "resolver" should "parse json into actions that can be executed" in {
    val parsed =
      JsonReader.buildProject(JsonInputFile.parse(simpleExample).right.get,
                              new S3JsonArtifact("artifact-bucket", "tmp/123"),
                              deploymentTypes)
    val deployRecipe = parsed.recipes("htmlapp-only")

    val host = Host("host1", stage = CODE.name, tags = Map("group" -> ""))
      .app(App("apache"))
    val lookup = stubLookup(host :: Nil)

    val resources = DeploymentResources(reporter, lookup, artifactClient)
    val tasks = Resolver.resolve(project(deployRecipe),
                                 parameters(deployRecipe),
                                 resources,
                                 region)
    ...
    taskList should be(
      List(
        S3Upload(Region("eu-west-1"),
                 "test",
                 Seq(
                   (new S3Path("artifact-bucket", "tmp/123/packages/htmlapp/"),
                    "CODE/htmlapp")),
                 publicReadAcl = true)
      ))
  }
  ...
  it should "prepare dependsOn actions correctly" in {
    ...
    val mainRecipe =
      Recipe("main",
             deploymentSteps = basePackageType.mkDeploymentStep(
               "main_init_action")(stubPackage(basePackageType)) :: Nil,
             dependsOn = List("one"))

    val resources =
      DeploymentResources(reporter, lookupSingleHost, artifactClient)
    val taskGraph = Resolver.resolve(project(mainRecipe, baseRecipe),
  ...
  it should "only include dependencies once" in {
    val basePackageType =
      stubDeploymentType(Seq("main_init_action", "init_action_two"))

    val indirectDependencyRecipe =
      Recipe("two",
             deploymentSteps = basePackageType.mkDeploymentStep(
               "init_action_two")(stubPackage(basePackageType)) :: Nil,
             dependsOn = List("one"))
    val mainRecipe =
      Recipe("main",
             deploymentSteps = basePackageType.mkDeploymentStep(
    ...
    val taskGraph = Resolver.resolve(
      project(mainRecipe, indirectDependencyRecipe, baseRecipe),
      parameters(mainRecipe),
      resources,
      region)
    DeploymentGraph.toTaskList(taskGraph) should be(
  ...
  it should "not throw an exception if no hosts found and only whole app recipes" in {
    val nonHostRecipe =
      Recipe("nonHostRecipe",
             deploymentSteps = basePackageType.mkDeploymentStep(
               "init_action_one")(stubPackage(basePackageType)) :: Nil,
             dependsOn = Nil)

    val resources =
      DeploymentResources(reporter, stubLookup(List()), artifactClient)
    Resolver.resolve(project(nonHostRecipe),
                     parameters(nonHostRecipe),
                     resources,
                     region)
  }
  ...
  it should "resolve tasks from multiple stacks" in {
      ...
      actionsMap = Map(
        "deploy" -> Action("deploy")(
          (pkg, resources, target) =>
            List(StubTask("stacked",
                          Region("eu-west-1"),
                          stack = Some(target.stack))))(StubActionRegister)),
      List("deploy")
    )
    val recipe =
      Recipe("stacked",
             deploymentSteps =
               List(pkgType.mkDeploymentStep("deploy")(stubPackage(pkgType))))

    val proj = project(recipe,
                       NamedStack("foo"),
                       NamedStack("bar"),
                       NamedStack("monkey"),
                       NamedStack("litre"))
    ...
    val taskGraph =
      Resolver.resolve(proj, parameters(recipe), resources, region)
    DeploymentGraph.toTaskList(taskGraph) should be(
      List(
        StubTask("stacked",
                 Region("eu-west-1"),
                 stack = Some(NamedStack("foo"))),
        StubTask("stacked",
                 Region("eu-west-1"),
                 stack = Some(NamedStack("bar"))),
        StubTask("stacked",
                 Region("eu-west-1"),
                 stack = Some(NamedStack("monkey"))),
        StubTask("stacked",
                 Region("eu-west-1"),
                 stack = Some(NamedStack("litre")))
      ))
  }

  it should "resolve tasks from multiple stacks into a parallel task graph" in {
    val pkgType = StubDeploymentType(
      actionsMap = Map("deploy" -> Action("deploy") {
        (pkg, resources, target) =>
          List(
            StubTask("stacked",
                     Region("eu-west-1"),
    ...
    successors should be(
      List(
        ValueNode(
          DeploymentTasks(List(StubTask("stacked",
                                        Region("eu-west-1"),
                                        stack = Some(NamedStack("foo")))),
                          "project -> foo")),
        ValueNode(
