...
class CloudFormationTest extends FlatSpec with Matchers with Inside {
      ...
      case List(updateTask, checkTask) =>
        inside(updateTask) {
          case UpdateCloudFormationTask(taskRegion,
            ...
            path should be(
              S3Path("artifact-bucket", "test/123/cloud-formation/cfn.json"))
  ...
  it should "ignore amiTags when amiParametersToTags and amiTags are provided" in {
    ...
    inside(
      ...
      case List(updateTask, _) =>
        inside(updateTask) {
          case UpdateCloudFormationTask(_,
                                        _,
                                        _,
                                        _,
                                        amiParamTags,
  ...
  "UpdateCloudFormationTask" should "substitute stack and stage parameters" in {
    val templateParameters =
      Seq(TemplateParameter("param1", false),
          TemplateParameter("Stack", false),
          TemplateParameter("Stage", false))
    val combined =
      UpdateCloudFormationTask.combineParameters(NamedStack("cfn"),
                                                 PROD,
                                                 templateParameters,
                                                 Map("param1" -> "value1"))
  ...
  it should "default required parameters to use existing parameters" in {
    val templateParameters =
      Seq(TemplateParameter("param1", true),
          TemplateParameter("param3", false),
          TemplateParameter("Stage", false))
    val combined =
      UpdateCloudFormationTask.combineParameters(NamedStack("cfn"),
                                                 PROD,
  ...
  it should "create new CFN stack names" in {
    ...
    nameToCallNewStack(
      LookupByTags(Map("Stack" -> "stackName",
                       "App" -> "appName",
                       "Stage" -> "STAGE"))) shouldBe
  ...
  "CloudFormationStackLookupStrategy" should "correctly create a LookupByName from deploy parameters" in {
    LookupByName(NamedStack("cfn"),
                 Stage("STAGE"),
                 "stackname",
                 prependStack = true,
                 appendStage = true) shouldBe
