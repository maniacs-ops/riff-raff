...
class DeployContextTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  it should "resolve a set of tasks" in {
    ...
    val parameters = DeployParameters(Deployer("tester"),
                                      Build("project", "1"),
                                      CODE,
                                      oneRecipeName)
    val resources =
      DeploymentResources(reporter, lookupSingleHost, artifactClient)
    val context = DeployContext(UUID.randomUUID(),
                                parameters,
                                project(baseRecipe),
                                resources,
                                region)
  ...
  it should "send a Info message when resolving tasks" in {
    val parameters = DeployParameters(Deployer("tester1"),
                                      Build("project", "1"),
                                      CODE,
                                      oneRecipeName)
    val reporter = DeployReporter.startDeployContext(
    ...
    DeployReporter.messages
      ...
      .subscribe(messages += _.stack.top)

    val resources =
      DeploymentResources(reporter, lookupSingleHost, artifactClient)
    val context = DeployContext(reporter.messageContext.deployId,
                                parameters,
                                project(baseRecipe),
                                resources,
                                region)
  ...
  it should "send taskStart and taskFinish messages for each task" in {
    val parameters = DeployParameters(Deployer("tester2"),
                                      Build("project", "1"),
                                      CODE,
                                      oneRecipeName)
  ...
  it should "bookend the messages with startdeploy and finishdeploy messages" in {
    val parameters = DeployParameters(Deployer("tester3"),
                                      Build("Project", "1"),
                                      CODE,
                                      oneRecipeName)
  ...
  case class MockStubPerHostDeploymentStep(description: String, apps: Seq[App])
  ...
  case class MockStubPerAppDeploymentStep(description: String, apps: Seq[App])
      extends DeploymentStep {
    def resolve(resources: DeploymentResources, target: DeployTarget) = {
      val task = mock[Task]
  ...
  val baseRecipe = Recipe(
    "one",
    deploymentSteps = basePackageType.mkDeploymentStep("init_action_one")(
      stubPackage(basePackageType)) :: Nil,
    dependsOn = Nil)

  val baseMockRecipe = Recipe(
    "one",
    deploymentSteps = MockStubPerAppDeploymentStep("init_action_one",
                                                   Seq(app1)) :: Nil,
    dependsOn = Nil)
