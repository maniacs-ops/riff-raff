...
class ReportingTest extends FlatSpec with Matchers {
  ...
  it should "build a one-op deploy report" in {
    val stacks = startDeployWrapper :: wrapper(
      infoMessageId,
      Some(deployMessageId),
      stack(infoMsg, deploy)) :: finishDeployWrapper :: Nil
    val report = DeployReport(stacks)

    report.isRunning should be(false)
    report.size should be(2)
    report shouldBe tree(deployMessageId,
                         startDeploy,
                         finishDep,
                         tree(infoMessageId, infoMsg))
  }
  ...
  it should "know it has failed" in {
    val failStacks = messageWrappers.take(4) ::: List(
      wrapper(UUID.randomUUID(),
              Some(infoMessageId),
              stack(failInfo, infoMsg, deploy)),
      wrapper(UUID.randomUUID(),
              Some(deployMessageId),
              stack(failDep, deploy)))
  ...
  def wrapper(id: UUID,
              ...
              stack: MessageStack): MessageWrapper = {
  ...
  def tree(id: UUID,
           ...
           trees: DeployReportTree*): DeployReportTree = {
    DeployReportTree(MessageState(startMessage, finishMessage, testTime, id),
                     trees.toList)
  }
  def tree(id: UUID,
           startMessage: StartContext,
           ...
           trees: DeployReportTree*): DeployReportTree = {
    DeployReportTree(MessageState(startMessage, failMessage, testTime, id),
                     trees.toList)
  }

  val parameters = DeployParameters(Deployer("Test reports"),
  ...
  val finishDeployWrapper =
    wrapper(finishDeployId, Some(deployMessageId), stack(finishDep, deploy))
  ...
  val messageWrappers: List[MessageWrapper] =
    startDeployWrapper ::
      ...
      wrapper(finishInfoId,
              Some(infoMessageId),
              stack(finishInfo, infoMsg, deploy)) ::
      finishDeployWrapper ::
      Nil
