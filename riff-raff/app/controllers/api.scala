            ...
            .map { entry =>
              ...
              val counter = try {
                ...
                case cce: ClassCastException =>
                  log.warn(
                    "Automatically marshalling an Int to a Long (you should only see this during unit tests)")
...
class Api(deployments: Deployments, deploymentTypes: Seq[DeploymentType])(
  ...
  object ApiJsonEndpoint {
    ...
    def apply[A](authenticatedRequest: ApiRequest[A])(
      ...
      jsonpCallback map { callback =>
        Ok("%s(%s)" format (callback, responseObject.toString))
    ...
    applicationForm
      ...
      .fold(
        ...
        applicationName => {
          ...
          val key = ApiKey(applicationName,
                           randomKey,
  ...
  def historyGraph = ApiJsonEndpoint.withAuthAccess { implicit request =>
    ...
    val pagination = deployment.DeployFilterPagination.fromRequest
      ...
      .withPageSize(None)
    val deployList =
      Deployments.getDeploys(filter, pagination.pagination, fetchLogs = false)
  ...
  def record2apiResponse(deploy: Record)(
      implicit request: ApiRequest[AnyContent]) =
    Json.obj(
      "time" -> deploy.time,
  ...
  def deploy = ApiJsonEndpoint("deploy", parse.json) { implicit request =>
    deployRequestReader
      ...
      .fold(
        valid = { deployRequest =>
          val (project, build, stage, recipeOption, hostsOption) =
            deployRequest
          ...
          deployments.deploy(
            params,
            requestSource = ApiRequestSource(request.apiKey)) match {
            case Right(deployId) =>
              Json.obj(
                "response" -> Json.obj(
                  "status" -> "ok",
