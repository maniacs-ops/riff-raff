...
case class DropDownMenuItem(title: String,
...
object Menu {
  lazy val menuItems = Seq(
    ...
    DropDownMenuItem(
      "Configuration",
      Seq(
        SingleMenuItem("Continuous Deployment",
    ...
    DropDownMenuItem(
      "Documentation",
      Seq(
        SingleMenuItem("Deployment Types",
                       routes.Application.documentation("magenta-lib/types")),
        SingleMenuItem("Validate configuration",
  ...
  lazy val loginMenuItem = SingleMenuItem("Login",
                                          routes.Login.loginAction(),
                                          identityRequired = false)
}

class Application(prismLookup: PrismLookup,
  ...
  def index = Action { implicit request =>
    ...
    val documentation = MarkDownParser.toHtml(
      ...
      linkRenderer = Some(new docs.MarkDownParser.CallLinkRenderer(link =>
  ...
  def makeAbsolute(resource: String, relative: String): String = {
    if (relative.isEmpty)
      ...
      if (relative.startsWith("../")) {
        makeAbsolute(resource.split("/").init.init.mkString("", "/", "/"),
  ...
  def getNextPrevFromHeader(
      resource: String,
      lines: List[String]): (Option[Link], Option[Link]) = {
          ...
          if (markDownLines.isEmpty) {
            ...
            resource match {
              case "magenta-lib/types" =>
                ...
                Ok(
                  views.html.documentation.markdownBlocks(request,
                                                          "Deployment Types",
                                                          breadcrumbs,
                                                          sections))
              case _ =>
                Ok(
                  views.html.documentation.markdown(request,
...
object Application extends Logging {
  ...
  def getMarkdownLines(resource: String)(
