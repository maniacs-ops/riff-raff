...
class DeployController(deployments: Deployments,
          ...
          form.action match {
            case "preview" =>
              ...
              Redirect(
                ...
                "previewStacks" -> parameters.stacks
  ...
  def viewUUID(uuidString: String, verbose: Boolean) = AuthAction {
    implicit request =>
      ...
      val stopFlag =
        if (record.isDone) false else deployments.getStopFlag(uuid)
  ...
  def preview(projectName: String,
              ...
              recipe: String,
              hosts: String,
              stacks: String) = AuthAction { implicit request =>
    val hostList = hosts.split(",").toList.filterNot(_.isEmpty)
    val stackList =
      stacks.split(",").toList.filterNot(_.isEmpty).map(NamedStack(_))
    val parameters = DeployParameters(Deployer(request.user.fullName),
                                      ...
                                      stackList,
                                      hostList)
    val previewId = LegacyPreviewController.startPreview(parameters,
                                                         prismLookup,
                                                         deploymentTypes)
    Ok(
      views.html.preview.json.preview(request, parameters, previewId.toString))
  }
            ...
            case Some(Success(preview)) =>
              try {
                ...
                case exception: Exception =>
                  Ok(
                    views.html.errorContent(
                      exception,
                      "Couldn't resolve preview information."))
              }
            case Some(Failure(exception)) =>
              Ok(
                views.html.errorContent(
                  exception,
                  "Couldn't retrieve preview information."))
            case None =>
              val duration =
                new org.joda.time.Duration(startTime, new DateTime())
              Ok(
                views.html.preview.json
                  .loading(request, duration.getStandardSeconds))
          }
        case _ =>
          val exception =
            new IllegalStateException("Future for preview wasn't found")
          Ok(
            views.html.errorContent(exception,
                                    "Couldn't resolve preview information."))
      }
    }
  ...
  def autoCompleteProject(term: String) = AuthAction {
    val possibleProjects = Builds.jobs
      .map(_.name)
  ...
  def autoCompleteBuild(project: String, term: String) = AuthAction {
    val possibleProjects = Builds
      ...
      .map { build =>
        val label = "%s [%s] (%s)" format (build.number, build.branchName, shortFormat
  ...
  def deployHistory(project: String, maybeStage: Option[String]) = AuthAction {
    request =>
      if (project.trim.isEmpty) {
        ...
        val filter =
          DeployFilter(projectName = Some(s"^$project$$"), stage = maybeStage)
        val records = Deployments
          .getDeploys(Some(filter),
  ...
  def builds = AuthAction {
    val header = Seq("Build Type Name",
                     "Build Number",
                     "Build Branch",
                     "Build Type ID",
                     "Build ID")
  ...
  def deployConfirmation(deployFormJson: String) = AuthAction {
    implicit request =>
      ...
      Ok(
        views.html.deploy.deployConfirmation(
  ...
  def deployAgainUuid(uuidString: String) = AuthAction { implicit request =>
    ...
    val params =
      DeployParameterForm(record.buildName,
                          record.buildId,
                          record.stage.name,
  ...
  def deployConfig(projectName: String, id: String) = AuthAction {
    implicit request =>
      def pathAndContent(
          artifact: S3Artifact): Either[S3Error, (S3Path, String)] = {
        val deployObjectPath = artifact.deployObject
        val deployObjectContent = S3Location.fetchContentAsString(
          deployObjectPath)(Configuration.artifact.aws.client)
        deployObjectContent.map(deployObjectPath -> _)
      }

      val build = Build(projectName, id)
      val deployObject = pathAndContent(
        S3YamlArtifact(build, Configuration.artifact.aws.bucketName))
        .orElse(pathAndContent(
          S3JsonArtifact(build, Configuration.artifact.aws.bucketName)))
  ...
  def deployFiles(projectName: String, id: String) = AuthAction {
    implicit request =>
      def pathAndContent(artifact: S3Artifact)
      ...
      deployObject.map {
        case (artifact, path, configFile) =>
          ...
          Ok(
            views.html.artifact
              .listFiles(request, projectName, id, relativeObjects))
      } getOrElse {
  ...
  def getArtifactFile(key: String) = AuthAction { implicit request =>
    val s3doc = Configuration.artifact.aws.client.getObject(
      new GetObjectRequest(Configuration.artifact.aws.bucketName, key))
    val stream = s3doc.getObjectContent
    val source: Source[ByteString, _] =
      StreamConverters.fromInputStream(() => stream)
    Ok.sendEntity(HttpEntity.Streamed(source, None, Some("")))
      .as(s3doc.getObjectMetadata.getContentType)
  }
