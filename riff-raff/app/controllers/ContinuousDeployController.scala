...
class ContinuousDeployController(prismLookup: PrismLookup)(
  ...
  def save = AuthAction { implicit request =>
    continuousDeploymentForm
      ...
      .fold(
        formWithErrors =>
          Ok(
            views.html.continuousDeployment.form(formWithErrors, prismLookup)),
        form => {
  ...
  def edit(id: String) = AuthAction { implicit request =>
    ContinuousDeploymentConfigRepository
      ...
      .map { config =>
        Ok(
          views.html.continuousDeployment.form(
            continuousDeploymentForm.fill(ConfigForm(config)),
            prismLookup))
      }
  ...
  def delete(id: String) = AuthAction { implicit request =>
    Form("action" -> nonEmptyText)
      ...
      .fold(
        errors => {}, {
          case "delete" =>
            ContinuousDeploymentConfigRepository.deleteContinuousDeployment(
              UUID.fromString(id))
        }
      )
    Redirect(routes.ContinuousDeployController.list())
