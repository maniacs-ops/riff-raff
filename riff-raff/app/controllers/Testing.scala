...
class Testing(prismLookup: PrismLookup)(implicit val messagesApi: MessagesApi,
  ...
  def reportTestPartial(take: Int, verbose: Boolean) = Action {
    implicit request =>
      ...
      val parameters = DeployParameters(Deployer("Simon Hildrew"),
      ...
      val testTask1 = new Task {
        ...
        def verbose =
          "A particularly verbose task description that lists some stuff, innit"
      ...
      def wrapper(parent: Option[UUID],
                  id: UUID,
                  messages: Message*): MessageWrapper = {
        MessageWrapper(MessageContext(logUUID, parameters, parent),
                       id,
    ...
    /var/folders/ZO/ZOSa3fR3FsCiU3jxetWKQU+++TQ/-Tmp-/sbt_5489e15/packages/riff-raff/riff-raff.jar"""),
        ...
        wrapper(Some(message.deployUUID),
                ...
                message.deploy),
        wrapper(Some(message.task1UUID),
                UUID.randomUUID(),
        ...
        wrapper(Some(message.deployUUID),
                ...
                message.deploy),
        wrapper(Some(message.task2UUID),
                message.infoUUID,
      ...
      val report = DeployRecord(new DateTime(),
                                logUUID,
                                parameters,
  ...
  def uuidList(limit: Int) = AuthAction { implicit request =>
    val allDeploys = Persistence.store
      .getDeployUUIDs()
      .toSeq
  ...
  def S3LatencyList(limit: Int, csv: Boolean) = AuthAction {
    implicit request =>
      val filter = DeployFilter.fromRequest
      val pagination = PaginationView.fromRequest
      val allDeploys = DocumentStoreConverter.getDeployList(filter,
                                                            pagination,
                                                            fetchLog = true)
