...
trait AuthorisationValidator {
  ...
  def authorisationError(id: UserIdentity): Option[String] = {
    if (!emailDomainWhitelist.isEmpty && !emailDomainWhitelist.contains(
          id.emailDomain)) {
      Some(
...
object ApiAuthAction {
  ...
  def apply[A](counter: Option[String], p: BodyParser[A])(
      f: ApiRequest[A] => Result): Action[A] = {
  ...
  def apply[A](p: BodyParser[A])(f: ApiRequest[A] => Result): Action[A] =
    this.apply(None, p)(f)
  def apply(counter: Option[String])(
      f: ApiRequest[AnyContent] => Result): Action[AnyContent] =
...
class Login(implicit val messagesApi: MessagesApi, val wsClient: WSClient)
  ...
  val validator = new AuthorisationValidator {
    ...
    def emailWhitelistEnabled =
      auth.whitelist.useDatabase || auth.whitelist.addresses.nonEmpty
    def emailWhitelistContains(email: String) = {
      val lowerCaseEmail = email.toLowerCase
      auth.whitelist.addresses.contains(lowerCaseEmail) ||
      (auth.whitelist.useDatabase && Persistence.store
  ...
  def loginAction = Action.async { implicit request =>
    ...
    GoogleAuth.redirectToGoogle(auth.googleAuthConfig, antiForgeryToken).map {
      _.withSession {
        request.session + (authConfig.antiForgeryKey -> antiForgeryToken)
      }
    }
  }
  ...
  def oauth2Callback = Action.async { implicit request =>
    request.session.get(authConfig.antiForgeryKey) match {
      ...
      case Some(token) =>
        GoogleAuth.validatedUserIdentity(auth.googleAuthConfig, token).map {
          identity =>
            require(validator.isAuthorised(identity),
                    validator
                      .authorisationError(identity)
                      .getOrElse("Unknown error"))
            val redirect =
              request.session.get(GoogleAuthFilters.LOGIN_ORIGIN_KEY) match {
  ...
  def authList = AuthAction { request =>
    Ok(
      views.html.auth
        .list(request, Persistence.store.getAuthorisationList.sortBy(_.email)))
  }
  ...
  def authSave = AuthAction { implicit request =>
    authorisationForm
      ...
      .fold(
        ...
        email => {
          val auth = AuthorisationRecord(email.toLowerCase,
