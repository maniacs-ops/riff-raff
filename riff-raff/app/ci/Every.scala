...
object Every extends Logging {
  ...
  def apply[T](frequency: Duration)(buildRetriever: => Observable[T])(
...
trait ContinuousIntegrationAPI {
  ...
  def succesfulBuildBatch(job: Job)(
      implicit ec: ExecutionContext): Observable[Iterable[CIBuild]]
  def tags(build: CIBuild)(
      implicit ec: ExecutionContext): Future[Option[List[String]]]
}

object FailSafeObservable extends Logging {
  def apply[T](f: Future[T], msg: => String)(
      implicit ec: ExecutionContext): Observable[T] =
