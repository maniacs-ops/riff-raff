...
class ContinuousDeployment(deployments: Deployments)
    extends Lifecycle
    with Logging {
  ...
  def buildCandidates(builds: Observable[CIBuild]): Observable[CIBuild] =
    (for {
      (_, buildsPerJobAndBranch) <- builds.groupBy(b =>
        (b.jobName, b.branchName))
  ...
  def runDeploy(params: DeployParameters) {
    if (conf.Configuration.continuousDeployment.enabled) {
      if (!ChangeFreeze.frozen(params.stage.name)) {
        ...
        try {
          deployments
            .deploy(params, requestSource = ContinuousDeploymentRequestSource)
            .left
            .foreach { error =>
              log.error(
                s"Couldn't continuously deploy $params: ${error.message}")
            }
        } catch {
        ...
        log.info(
          s"Due to change freeze, continuous deployment is skipping ${params.toString}")
      }
    } else
...
object ContinuousDeployment extends Logging {
  ...
  def getMatchesForSuccessfulBuilds(
      build: CIBuild,
      configs: Iterable[ContinuousDeploymentConfig])
  ...
  def getDeployParams(configBuildTuple: (ContinuousDeploymentConfig, CIBuild))
    : DeployParameters = {
