...
class TasksRunner(stopFlagAgent: Agent[Map[UUID, String]])
  ...
  def receive = {
    case RunDeployment(uuid, tasks, rootReporter, queueTime) =>
      ...
      try {
        rootReporter.infoContext(s"Deploying ${tasks.name}") {
          deployReporter =>
            try {
              tasks.tasks.zipWithIndex.foreach {
                case (task, index) =>
                  ...
                  if (stopFlagAsker) {
                    ...
                    deployReporter.fail(stopMessage,
                                        DeployStoppedException(stopMessage))
                  } else {
                    try {
                      log.debug(s"Running task $taskId")
                      deployMetricsProcessor ! TaskStart(uuid,
                                                         taskId,
                                                         queueTime,
                                                         new DateTime())
                      ...
                      deployMetricsProcessor ! TaskComplete(uuid,
                                                            taskId,
                                                            new DateTime())
                    }
                  }
...
object TasksRunner {
  trait Message
  case class RunDeployment(uuid: UUID,
