...
class Deployments(deploymentEngine: DeploymentEngine) extends Logging {
  ...
  def deploy(requestedParams: DeployParameters,
             requestSource: RequestSource): Either[Error, UUID] = {
      ...
      val params =
        if (requestedParams.build.id != "lastSuccessful")
          ...
          Builds
            ...
            .map { latestId =>
              requestedParams.copy(
                build = requestedParams.build.copy(id = latestId))
            }
...
object Deployments extends Logging with Lifecycle {
  ...
  private val messagesSubscription: Subscription =
    DeployReporter.messages.subscribe { wrapper =>
      try {
        ...
        case NonFatal(t) =>
          log.error(s"Exception thrown whilst processing update with $wrapper",
                    t)
      }
    }
  def init() {}
  ...
  def getDeploys(filter: Option[DeployFilter] = None,
    ...
    getDatastoreDeploys(filter, pagination, fetchLogs = fetchLogs).toList
      .sortWith { _.time.getMillis < _.time.getMillis }
  }
