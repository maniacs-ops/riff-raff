...
object Preview extends Loggable {
  def apply(artifact: S3YamlArtifact,
    ...
    val validatedGraph = for {
      deploymentGraph <- Resolver.resolveDeploymentGraph(yamlConfig,
                                                         deploymentTypes,
                                                         parameters.selector)
      flattenedGraph = DeploymentGraphActionFlattening.flattenActions(
        deploymentGraph)
...
case class Preview(
    graph: Validated[ConfigErrors, Graph[(DeploymentKey, DeploymentTasks)]],
    parameters: DeployParameters)
