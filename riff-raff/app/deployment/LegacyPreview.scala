...
case class LegacyPreviewResult(future: Future[LegacyPreview],
...
object LegacyPreviewController {
  ...
  def startPreview(parameters: DeployParameters,
    ...
    val previewFuture = Future {
      LegacyPreview(parameters, resources, deploymentTypes)
    }
    Await.ready(agent.alter {
      _ + (previewId -> LegacyPreviewResult(previewFuture))
    }, 30.second)
    previewId
  }

  def getPreview(id: UUID,
                 parameters: DeployParameters): Option[LegacyPreviewResult] =
    agent().get(id)
}
...
object LegacyPreview {
  ...
  /**
    ...
    */
  def getProject(build: Build,
                 resources: DeploymentResources,
                 deploymentTypes: Seq[DeploymentType]): Project = {
    val yamlArtifact = S3YamlArtifact(build, artifact.aws.bucketName)
    if (yamlArtifact.deployObject
          .fetchContentAsString()(resources.artifactClient)
          .isRight)
      throw new IllegalArgumentException(
        "Tried to preview 'riff-raff.yaml' with old preview tool")
    val s3Artifact = S3JsonArtifact(build, artifact.aws.bucketName)
    val json = S3JsonArtifact.fetchInputFile(s3Artifact)(
      resources.artifactClient,
      resources.reporter)
    json.fold[Project](
      e => resources.reporter.fail(s"Unable to build preview, $e"),
  ...
  def apply(parameters: DeployParameters,
    ...
    val project =
      LegacyPreview.getProject(parameters.build, resources, deploymentTypes)
    LegacyPreview(project,
                  parameters,
                  resources,
                  Region(target.aws.deployJsonRegionName))
  }
}
...
case class LegacyPreview(project: Project,
                         ...
                         resources: DeploymentResources,
                         region: Region) {
  lazy val stacks = Resolver.resolveStacks(project,
                                           parameters,
                                           resources.reporter) collect {
    case NamedStack(s) => s
  }
  ...
  lazy val recipeTasks =
    Resolver.resolveDetail(project, parameters, resources, region)
  lazy val tasks = recipeTasks.flatMap(_.tasks)

  def taskHosts(taskList: List[MagentaTask]) =
    taskList
      .flatMap(_.taskHost)
      .filter(resources.lookup.hosts.all.contains)
      .distinct

  lazy val hosts = taskHosts(tasks)
  lazy val allHosts = {
    val tasks = Resolver.resolve(
      project,
      parameters.copy(recipe = RecipeName(recipe), hostList = Nil),
      resources,
      region)
  ...
  def withRecipe(newRecipe: String) =
    routes.DeployController.preview(projectName,
                                    build,
                                    stage,
                                    newRecipe,
                                    "",
