...
case class DeployFilter(projectName: Option[String] = None,
  ...
  def withProjectName(projectName: Option[String]) =
    this.copy(projectName = projectName)
  ...
  def withMaxDaysAgo(maxDaysAgo: Option[Int]) =
    this.copy(maxDaysAgo = maxDaysAgo)
    ...
    val statusType = try {
      ...
      case t: Throwable =>
        throw new IllegalArgumentException(
          "Unknown value for status parameter")
    }
...
case class HostFilter(stage: Option[String] = None,
  ...
  lazy val queryStringParams: List[(String, String)] = {
    Nil ++
      ...
      (if (hostList.isEmpty) None
       else Some("hostList" -> hostList.mkString(",")))
  }
}
  ...
  def fromRequest(implicit r: RequestHeader): HostFilter = {
    ...
    def listParam(s: String): List[String] =
      r.queryString
        .get(s)
        .getOrElse(Nil)
        .flatMap(_.split(",").map(_.trim).filter(!_.isEmpty))
        .toList
...
trait Pagination extends QueryStringBuilder {
  ...
  def lowerBound: Int =
    math.max(1,
             pageCount
               .map(pageCount => math.min(page - 2, pageCount - 4))
               .getOrElse(page - 4))
  def upperBound: Int =
    pageCount
      .map(pageCount => math.min(pageCount, math.max(page + 2, 5)))
      .getOrElse(page)

  def pageCount: Option[Int] =
    itemCount.map(itemCount => math.ceil(itemCount.toDouble / pageSize).toInt)

  def hasPrevious = page != 1
  def hasNext = true
  def hasLast = pageCount.isDefined

  def previous(base: Call) =
    if (hasPrevious) base.appendQueryParams(withPage(page - 1).q) else DISABLED
  def next(base: Call) =
    if (hasNext) base.appendQueryParams(withPage(page + 1).q) else DISABLED

  def first(base: Call) = toPage(base, 1)
  def last(base: Call) = pageCount.map(toPage(base, _)).getOrElse(DISABLED)

  def toPage(base: Call, newPage: Int) =
    base.appendQueryParams(withPage(newPage).q)
}
...
case class PaginationView(
  ...
  def isDefault =
    pageSize == PaginationView.DEFAULT_PAGESIZE && page == PaginationView.DEFAULT_PAGE
...
case class DeployFilterPagination(filter: DeployFilter,
  ...
  def replacePagination(f: PaginationView => PaginationView) =
    this.copy(pagination = f(pagination))

  def withProjectName(projectName: Option[String]) =
    this.copy(filter = filter.withProjectName(projectName))
  def withStage(stage: Option[String]) =
    this.copy(filter = filter.withStage(stage))
  def withDeployer(deployer: Option[String]) =
    this.copy(filter = filter.withDeployer(deployer))
  def withStatus(status: Option[RunState.Value]) =
    this.copy(filter = filter.withStatus(status))
  def withPage(page: Int): DeployFilterPagination =
    this.copy(pagination = pagination.withPage(page))
  def withPageSize(size: Option[Int]) =
    this.copy(pagination = pagination.withPageSize(size))
...
object DeployFilterPagination {
  def fromRequest(implicit r: RequestHeader): DeployFilterPagination = {
    DeployFilterPagination(DeployFilter.fromRequest.getOrElse(DeployFilter()),
                           PaginationView.fromRequest)
  }
}
