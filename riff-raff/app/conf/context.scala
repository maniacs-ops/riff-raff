...
class Configuration(val application: String,
                    val webappConfDirectory: String = "env")
    extends Logging {
  protected val configuration =
    ConfigurationFactory.getConfiguration(application, webappConfDirectory)
  ...
  object auth {
    lazy val domains: List[String] =
      configuration.getStringPropertiesSplitByComma("auth.domains")
    object whitelist {
      lazy val useDatabase: Boolean = configuration.getStringProperty(
        "auth.whitelist.useDatabase",
        "false") == "true"
      lazy val addresses: List[String] =
        configuration.getStringPropertiesSplitByComma(
          "auth.whitelist.addresses")
    }
    lazy val clientId: String = configuration
      .getStringProperty("auth.clientId")
      .getOrException("No client ID configured")
    lazy val clientSecret: String = configuration
      .getStringProperty("auth.clientSecret")
      .getOrException("No client secret configured")
    ...
    lazy val domain: Option[String] =
      configuration.getStringProperty("auth.domain")
    lazy val googleAuthConfig = GoogleAuthConfig(auth.clientId,
                                                 auth.clientSecret,
                                                 auth.redirectUrl,
                                                 auth.domain)
    lazy val superusers: List[String] =
      configuration.getStringPropertiesSplitByComma("auth.superusers")
  }

  object concurrency {
    lazy val maxDeploys =
      configuration.getIntegerProperty("concurrency.maxDeploys", 8)
  }

  object continuousDeployment {
    lazy val enabled = configuration.getStringProperty(
  ...
  object freeze {
    ...
    lazy val startDate = configuration
      .getStringProperty("freeze.startDate")
      .map(formatter.parseDateTime)
    lazy val endDate = configuration
      .getStringProperty("freeze.endDate")
      .map(formatter.parseDateTime)
    ...
    lazy val stages =
      configuration.getStringPropertiesSplitByComma("freeze.stages")
  }

  object housekeeping {
    lazy val summariseDeploysAfterDays = configuration.getIntegerProperty(
      "housekeeping.summariseDeploysAfterDays",
      90)
    lazy val hour = configuration.getIntegerProperty("housekeeping.hour", 4)
    lazy val minute =
      configuration.getIntegerProperty("housekeeping.minute", 0)
  }

  object logging {
    lazy val verbose = configuration
      .getStringProperty("logging")
      .exists(_.equalsIgnoreCase("VERBOSE"))
  }

  object lookup {
    lazy val prismUrl = configuration
      .getStringProperty("lookup.prismUrl")
      .getOrException("Prism URL not specified")
    lazy val timeoutSeconds =
      configuration.getIntegerProperty("lookup.timeoutSeconds", 30)
  }

  object mongo {
    lazy val isConfigured = uri.isDefined
    lazy val uri = configuration.getStringProperty("mongo.uri")
    lazy val collectionPrefix =
      configuration.getStringProperty("mongo.collectionPrefix", "")
  }

  object stages {
    lazy val order = configuration
      .getStringPropertiesSplitByComma("stages.order")
      .filterNot("" ==)
  ...
  object artifact {
    object aws {
      ...
      lazy val accessKey =
        configuration.getStringProperty("artifact.aws.accessKey")
      lazy val secretKey =
        configuration.getStringProperty("artifact.aws.secretKey")
      lazy val credentialsProvider =
        credentialsProviderChain(accessKey, secretKey)
      lazy val regionName =
        configuration.getStringProperty("artifact.aws.region", "eu-west-1")
  ...
  object build {
    lazy val pollingPeriodSeconds =
      configuration.getIntegerProperty("build.pollingPeriodSeconds", 10)
    object aws {
      implicit lazy val bucketName =
        configuration.getStringProperty("build.aws.bucketName")
      lazy val accessKey =
        configuration.getStringProperty("build.aws.accessKey")
      lazy val secretKey =
        configuration.getStringProperty("build.aws.secretKey")
      lazy val credentialsProvider =
        credentialsProviderChain(accessKey, secretKey)
      lazy val regionName =
        configuration.getStringProperty("build.aws.region", "eu-west-1")
      implicit lazy val client: AmazonS3 = AmazonS3ClientBuilder
        .standard()
        .withCredentials(credentialsProvider)
        .withRegion(regionName)
  ...
  object tag {
    object aws {
      implicit lazy val bucketName =
        configuration.getStringProperty("tag.aws.bucketName")
      ...
      lazy val credentialsProvider =
        credentialsProviderChain(accessKey, secretKey)
      lazy val regionName =
        configuration.getStringProperty("tag.aws.region", "eu-west-1")
      implicit lazy val client: AmazonS3 = AmazonS3ClientBuilder
        .standard()
        .withCredentials(credentialsProvider)
        .withRegion(regionName)
  ...
  object target {
    object aws {
      lazy val deployJsonRegionName = configuration.getStringProperty(
        "target.aws.deployJsonRegion",
        "eu-west-1")
    }
  }
  ...
  def credentialsProviderChain(
      ...
      secretKey: Option[String]): AWSCredentialsProviderChain =
  ...
  object urls {
    lazy val publicPrefix: String = configuration.getStringProperty(
      "urls.publicPrefix",
      "http://localhost:9000")
  }
...
object Configuration
    extends Configuration("riff-raff", webappConfDirectory = "env")
...
object PlayRequestMetrics
    extends com.gu.management.play.RequestMetrics.Standard
...
object DeployMetrics extends Lifecycle {
  ...
  object DeployStart
      extends CountMetric("riffraff",
                          "start_deploy",
                          "Start deploy",
                          "Number of deploys that are kicked off")
  ...
  val messageSub = DeployReporter.messages.subscribe(message => {
    message.stack.top match {
      ...
      case FailContext(Deploy(parameters)) =>
        DeployFail
          .recordCount(1) // TODO this metric appears to be broken, never gets incremented
        runningDeploys -= message.context.deployId
...
object DatastoreMetrics {
  ...
  val collectionStats =
    ScheduledAgent(5 seconds, 5 minutes, Map.empty[String, CollectionStats]) {
      map =>
        Persistence.store.collectionStats
    }
  def dataSize: Long =
    collectionStats().values.map(_.dataSize).foldLeft(0L)(_ + _)
  def storageSize: Long =
    collectionStats().values.map(_.storageSize).foldLeft(0L)(_ + _)
  def deployCollectionCount: Long =
    collectionStats()
      .get("%sdeployV2" format Configuration.mongo.collectionPrefix)
      .map(_.documentCount)
      .getOrElse(0L)
  ...
  val all = Seq(DatastoreRequest,
                MongoDataSize,
                MongoStorageSize,
                MongoDeployCollectionCount)
}

object LoginCounter
    extends CountMetric("webapp",
                        "login_attempts",
                        "Login attempts",
                        "Number of attempted logins")

object FailedLoginCounter
    extends CountMetric("webapp",
                        "failed_logins",
                        "Failed logins",
...
object Switches {
  ...
  val all
    : Seq[Switchable] = ShutdownWhenInactive.switch :: Healthcheck.switch :: Deployments.enableSwitches
}
