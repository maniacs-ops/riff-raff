...
case class ParamDoc(name: String,
                    documentation: String,
                    defaultLegacy: Option[String],
                    default: Option[String])
case class DeployTypeDocs(documentation: String,
                          actions: Seq[ActionDoc],
                          params: Seq[ParamDoc])

object DeployTypeDocs {
  def defaultFromParam(fakePackage: DeploymentPackage,
                       param: Param[_]): Option[String] = {
    ...
    (param.defaultValue,
     param.defaultValueFromContext.map(_(fakePackage, fakeTarget))) match {
  ...
  def generateDocs(deploymentTypes: Seq[DeploymentType])
    ...
    deploymentTypes.sortBy(_.name).map { dt =>
      ...
      val legacyPackage = DeploymentPackage("<packageName>",
                                            Seq(App("<app>")),
                                            Map.empty,
                                            dt.name,
                                            ...
                                            legacyConfig = true,
                                            deploymentTypes)
      val newPackage = DeploymentPackage("<packageName>",
                                         Seq(App("<app>")),
                                         Map.empty,
                                         dt.name,
                                         S3Path("<bucket>", "<prefix>"),
