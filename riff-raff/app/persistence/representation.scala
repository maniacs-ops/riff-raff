...
object DeployRecordDocument extends MongoSerialisable[DeployRecordDocument] {
  def apply(uuid: String,
            startTime: DateTime,
            parameters: ParametersDocument,
            status: String): DeployRecordDocument = {
    DeployRecordDocument(UUID.fromString(uuid),
                         Some(uuid),
                         startTime,
                         parameters,
                         RunState.withName(status))
  }
  implicit val deployFormat: MongoFormat[DeployRecordDocument] =
    new DeployMongoFormat
  private class DeployMongoFormat extends MongoFormat[DeployRecordDocument] {
    def toDBO(a: DeployRecordDocument) = {
      val fields: List[(String, Any)] =
        ...
        ) ++ a.stringUUID.map("stringUUID" ->) ++ a.summarised.map(
          "summarised" -> _) ++
          a.totalTasks.map("totalTasks" ->) ++ a.completedTasks.map(
          "completedTasks" ->) ++
...
object ParametersDocument extends MongoSerialisable[ParametersDocument] {
  implicit val parametersFormat: MongoFormat[ParametersDocument] =
    new ParameterMongoFormat
    ...
    def fromDBO(dbo: MongoDBObject) =
      Some(
        ParametersDocument(
          ...
          stacks = dbo
            .getAsOrElse[MongoDBList]("stacks", MongoDBList())
            .map(_.asInstanceOf[String])
            .toList,
          hostList =
            dbo.as[MongoDBList]("hostList").map(_.asInstanceOf[String]).toList,
          tags = dbo
            .as[DBObject]("tags")
            .map(entry => (entry._1, entry._2.asInstanceOf[String]))
            .toMap,
          selector = dbo
            .getAs[DBObject]("selector")
            .map(DeploymentSelectorDocument.from)
            .getOrElse(AllDocument)
        ))
  }
}
...
object LogDocument extends MongoSerialisable[LogDocument] {
  ...
  def apply(deploy: UUID,
            id: UUID,
            parent: Option[UUID],
            document: Message,
            time: DateTime): LogDocument = {
          ...
          apps = dbo
            ...
            .map { dbo =>
              (dbo.getAs[String]("name"),
               ...
               dbo.getAs[String]("app")) match {
                case (Some(name), None, None) => App(name)
                case (None, Some(stack), Some(app)) => App(app)
                case other =>
                  throw new IllegalArgumentException(
                    s"Don't know how to construct App from tuple $other")
              }
            }
            .toSet,
          stage = dbo.as[String]("stage"),
    ...
    def fromDBO(dbo: MongoDBObject) =
      Some(
        TaskDetail(
          ...
          taskHosts = dbo
            .as[MongoDBList]("taskHosts")
            .flatMap(item => host.fromDBO(item.asInstanceOf[DBObject]))
            .toList
        ))
  }
...
sealed trait MessageDocument {
  def asMessage(params: DeployParameters,
                originalMessage: Option[Message] = None): Message
...
case class TaskListDocument(taskList: List[TaskDetail])
  ...
  def asMessage(params: DeployParameters, originalMessage: Option[Message]) =
    TaskList(taskList)
  override lazy val dboFields = List(
    "taskList" -> taskList.map(DetailConversions.taskDetail.toDBO))
}
case class TaskRunDocument(task: TaskDetail) extends MessageDocument {
  def asMessage(params: DeployParameters, originalMessage: Option[Message]) =
    TaskRun(task)
  override lazy val dboFields = List(
    "task" -> DetailConversions.taskDetail.toDBO(task))
}
...
case class FailDocument(text: String, detail: ThrowableDetail)
  ...
  override lazy val dboFields = List(
    "text" -> text,
    "detail" -> DetailConversions.throwableDetail.toDBO(detail))
}
  ...
  def from(dbo: DBObject): MessageDocument = {
    ...
    dbo.as[String]("_typeHint") match {
      ...
      case "persistence.TaskListDocument" =>
        TaskListDocument(
          dbo
            .as[MongoDBList]("taskList")
            .flatMap(dbo => taskDetail.fromDBO(dbo.asInstanceOf[DBObject]))
            .toList)
      case "persistence.TaskRunDocument" =>
        TaskRunDocument(taskDetail.fromDBO(dbo.as[DBObject]("task")).get)
      case "persistence.InfoDocument" => InfoDocument(dbo.as[String]("text"))
      case "persistence.CommandOutputDocument" =>
        CommandOutputDocument(dbo.as[String]("text"))
      case "persistence.CommandErrorDocument" =>
        CommandErrorDocument(dbo.as[String]("text"))
      case "persistence.VerboseDocument" =>
        VerboseDocument(dbo.as[String]("text"))
      case "persistence.FailDocument" =>
        FailDocument(dbo.as[String]("text"),
                     throwableDetail.fromDBO(dbo.as[DBObject]("detail")).get)
      ...
      case "persistence.WarningDocument" =>
        WarningDocument(dbo.as[String]("text"))
      case hint =>
        throw new IllegalArgumentException(
...
case class DeploymentKeyDocument(name: String,
                                 action: String,
                                 stack: String,
                                 region: String)

object DeploymentKeyDocument extends MongoSerialisable[DeploymentKeyDocument] {
  implicit val deploymentKeyFormat: MongoFormat[DeploymentKeyDocument] =
    new DeploymentKeyFormat
  private class DeploymentKeyFormat
      extends MongoFormat[DeploymentKeyDocument] {
    ...
    dbo.as[String]("_typeHint") match {
      ...
      case "persistence.DeploymentKeysSelectorDocument" =>
        DeploymentKeysSelectorDocument(
          dbo
            .as[List[DBObject]]("keys")
            .flatMap(dbo => DeploymentKeyDocument.fromDBO(dbo))
        )
    }
  }
}
