...
object RestrictionChecker {
  ...
  def configsThatPreventDeployment(
      restrictions: RestrictionsConfigRepository,
      projectName: String,
      stage: String,
  ...
  def sourceMatches(config: RestrictionConfig,
                    source: RequestSource): Boolean = {
    source match {
      case ContinuousDeploymentRequestSource => config.continuousDeployment
      case UserRequestSource(identity) =>
        config.whitelist.contains(identity.email)
      case _ => false
    }
  }
