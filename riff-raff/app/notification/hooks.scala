      ...
      (method match {
        ...
        case POST =>
          postBody
            .map { t =>
              val body =
                new HookTemplate(t, record, urlEncode = false).Template
                  .run()
                  .get
              val json = Try {
                Json.parse(body)
            ...
            .getOrElse(
              urlRequest.post(Map[String, Seq[String]](
                ...
                "tags" -> record.parameters.tags.toSeq.map {
                  case ((k, v)) => s"$k:$v"
                }
              ))
            )
          ...
          log.info(
            s"HTTP status code ${response.status} from ${urlRequest.url}")
          log.debug(
...
object HookConfig {
  def apply(projectName: String,
            ...
            url: String,
            enabled: Boolean,
            updatedBy: String): HookConfig =
    HookConfig(UUID.randomUUID(),
               projectName,
               stage,
               url,
               enabled,
...
class HooksClient(wsClient: WSClient) extends Lifecycle with Logging {
  ...
  val actor = try {
    Some(
      system.actorOf(Props(classOf[HooksClientActor], wsClient),
                     "hook-client"))
  } catch {
    case t: Throwable =>
...
class HooksClientActor(implicit wsClient: WSClient)
    extends Actor
    with Logging {
