...
object ScheduledAgent extends Lifecycle {
  ...
  def apply[T](initialDelay: FiniteDuration, frequency: FiniteDuration)(
      block: => T): ScheduledAgent[T] = {
    ScheduledAgent(initialDelay, frequency, block)(_ => block)
  }

  def apply[T](initialDelay: FiniteDuration,
               frequency: FiniteDuration,
               initialValue: T)(block: T => T): ScheduledAgent[T] = {
    ScheduledAgent(
      initialValue,
  ...
  def apply[T](initialValue: T,
               updates: ScheduledAgentUpdate[T]*): ScheduledAgent[T] = {
...
case class PeriodicScheduledAgentUpdate[T](block: T => T,
...
object DailyScheduledAgentUpdate {
  def apply[T](timeOfDay: LocalTime)(
      block: T => T): DailyScheduledAgentUpdate[T] =
    DailyScheduledAgentUpdate(block, timeOfDay)
}

class ScheduledAgent[T](system: ActorSystem,
                        initialValue: T,
