...
class RestrictionCheckerTest extends FlatSpec with Matchers {
  ...
  val configs: Seq[RestrictionConfig] = Seq(
    ...
    makeConfig("testProject1",
               "CODE",
               cd = true,
               whitelist = Seq("test.user@example.com")),
  ...
  it should "report superusers when locked" in {
    ...
    RestrictionChecker.isEditable(
      ...
      "Locked by test.creator@example.com - can also be modified by superuser@example.com"))
  }
  ...
  it should "match when a user request and user is present in whitelist" in {
    RestrictionChecker.sourceMatches(
      config.copy(
        whitelist = Seq("user1@example.com",
  ...
  def makeConfig(projectName: String,
    ...
    RestrictionConfig(UUID.randomUUID,
                      ...
                      cd,
                      "")
  def makeUser(email: String) =
    UserIdentity("1234", email, "Test", "User", 123344567845L, None)
