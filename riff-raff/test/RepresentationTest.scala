...
class RepresentationTest
  ...
  "LogDocument" should "serialise all message types to BSON" in {
    val messages = Seq(deploy,
                       ...
                       failInfo,
                       failDep)
    val documents = messages.map(
      LogDocument(testUUID,
                  UUID.randomUUID(),
  ...
  it should "not change without careful thought and testing of migration" in {
    ...
    messageJsonMap.foreach {
      case (message, json) =>
        val logDocument =
          LogDocument(testUUID, id, Some(parentId), message, time)
        ...
        val ungratedDeployDocument =
          LogDocument.fromDBO(new MongoDBObject(ungratedDBObject))
  ...
  it should "never change without careful thought and testing of migration" in {
    ...
    val deployDocument =
      RecordConverter(comprehensiveDeployRecord).deployDocument
    ...
    val ungratedDeployDocument =
      DeployRecordDocument.fromDBO(new MongoDBObject(ungratedDBObject))
  ...
  "ContinuousDeploymentConfig" should "never change without careful thought and testing of migration" in {
    ...
    val ungratedConfig =
      ContinuousDeploymentConfig.fromDBO(new MongoDBObject(ungratedDBObject))
    ...
    val ungratedV2Config =
      ContinuousDeploymentConfig.fromDBO(new MongoDBObject(ungratedV2DBObject))
  ...
  it should "not change DeployIdsFilterDocument without careful thought and testing of migration" in {
    ...
    val keysSelectorDocument = DeploymentKeysSelectorDocument(
      List(
        DeploymentKeyDocument("testName",
                              "testAction",
                              "testStack",
                              "testRegion")))
    ...
    val ungratedDeployIdsDBOject =
      JSON.parse(keySelectorJson).asInstanceOf[DBObject]
    ungratedDeployIdsDBOject.toString shouldBe deployKeysDump

    val ungratedDeployKeys =
      DeploymentSelectorDocument.from(ungratedDeployIdsDBOject)
