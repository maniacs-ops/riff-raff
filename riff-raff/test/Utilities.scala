...
trait Utilities {
  def compareJson(from: String, to: String) =
    JsonDiff.diff(Json.parse(from), Json.parse(to), remember = true)
}
...
trait PersistenceTestInstances {
  ...
  lazy val parameters = DeployParameters(Deployer("Tester"),
                                         ...
                                         selector = All)
  lazy val testParamsWithHosts =
    parameters.copy(hostList = List("host1", "host2"))
  lazy val testRecord = DeployRecord(testTime,
                                     testUUID,
                                     parameters,
                                     Map("branch" -> "master"),
                                     messageWrappers)
  ...
  def wrapper(parentId: Option[UUID], stack: MessageStack): MessageWrapper = {
    MessageWrapper(MessageContext(testUUID, parameters, parentId),
                   UUID.randomUUID(),
                   stack)
  }
  ...
  val finishDeployWrapper =
    wrapper(UUID.randomUUID(), Some(deployMessageId), stack(finishDep, deploy))
